...
В современных версиях _Quarto_, для выполняемых блоков кода языка _Julia_ есть возможность их выполнения непосредственно в julia (по умолчанию используется _jupyter_). Для этого в заголовке необходимо прописать опцию `engine: julia`; этого достаточно для Quarto документов с блоками кода _julia_. Если же в qmd-документе, при этом, присутствуют одновременно ещё _python_, или _r_ блоки, то дополнительно потребуется выполнить следующую команду в текущей директории:
```sh
julia --project=. -e 'import Pkg; Pkg.add("QuartoNotebookRunner"); Pkg.add("PythonCall"); Pkg.add("CondaPkg"); Pkg.add("RCall");'
julia --project=. -e 'import Pkg; Pkg.build("QuartoNotebookRunner"); Pkg.build("PythonCall"); Pkg.build("CondaPkg"); Pkg.build("RCall");'
...
julia> import Pkg
julia> Pkg.activate(".")
julia> Pkg.add("QuartoNotebookRunner")
julia> Pkg.add("PythonCall")
julia> Pkg.add("CondaPkg")
julia> Pkg.add("RCall")
julia> Pkg.build("QuartoNotebookRunner")
...
Также потребуется установить используемые в выполняемых блоках кода Julia пакеты: `Pkg.add("Plots")`.
...
![](basic-gnuplot.png){height=5.625in, width=7in}
...
plot(x, y,
     label="1/(x-1)^(2/3)",
     title="Plot of 1/(x-1)^(2/3)", # also can be `:none`
     xlims=(xl, xh),
     xlabel="x",
     ylabel="y",
...
# display plot at screen
#gui()

savefig("basic-julia.png")
```

![](basic-julia.png){height=5.625in, width=7in}
