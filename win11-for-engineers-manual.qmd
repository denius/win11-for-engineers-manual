...
# Активация текущего проекта, если он не в системной директории
if !(Base.active_project() !== nothing && !startswith(Base.active_project(), Base.Sys.BINDIR))
    Pkg.activate(".")
end

# Проверка и установка пакетов через Pkg
function install_if_missing(pkg::String)
    if !haskey(Pkg.installed(), pkg)
        Pkg.add(pkg)
    end
end

install_if_missing("CondaPkg")
install_if_missing("PythonCall")
install_if_missing("Plots")
install_if_missing("LaTeXStrings")

import CondaPkg

# Проверка и установка библиотек Python через CondaPkg
function conda_install_if_missing(pkg::String)
    conda_packages = CondaPkg.conda_list()
    if !haskey(conda_packages, pkg)
        CondaPkg.add(pkg)
    end
end

conda_install_if_missing("numpy")
conda_install_if_missing("matplotlib")
