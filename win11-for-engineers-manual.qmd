...
К особенностям работы нейронных сетей типа трансформер можно отнести:
...
* несколько приближённо, но можно сказать, что каждый слой НС представляет собой матрицу, и прохождение информации через слой является операцией умножения этой матрицы на вектор, содержащий закодированную информацию;
...
### Примеры запросов к ЯМ/LLM {.unnumbered .unlisted}
...
### Структуризация запросов и Markdown {.unnumbered .unlisted}
...
За один запрос модель вряд ли сможет выполнить сложную задачу, поэтому задачу необходимо разбить на этапы, как в примере со справочником. Для того, чтобы модель за раз смогла выполнить сложную задачу, необходимо составлять сложные, состоящие из многих пунктов запросы, с примерами, с критериями, и т.п.
...
### Задание и отображение математических выражений {.unnumbered .unlisted}
...
### *Отступление* об особенностях запросов для локальных LLM {.unnumbered .unlisted}

В LLM предыдущих поколений качество запроса было критически важно для успешности решения задач. Современные LLM, обученные методом *Reinforcement Learning*, уже не столь сильно зависят от ясности, явности, и однозначности запросов. Но, тем не менее, все онлайн-сервисы языковых моделей при работе автоматически добавляют к пользовательскому запросу так называемый *системный запрос* (*System PROMPT*), который улучшает и структурирует "мыслительную цепочку" языковой модели.

Некоторые коммерческие сервисы открывают свои _системные запросы_; они были собраны, и представлены в библиотеке <https://github.com/elder-plinius/CL4R1T4S>. По текстам системных запросов видно, что, во-первых, они очень объёмные --- десятки килобайт, это многие страницы текста; а во-вторых, что достаточно значительная часть этих десятков килобайт запроса состоит именно в попытке улучшить структуру мышления модели; остальная часть системного запроса указывает ограничения на выполнение для безопасности ответов.

При локальном применении LLM некий *системный запрос* тоже добавляется, но он совершенно не столь широк и универсален как системные запросы коммерческих систем, и, часто представляет собой буквально одно предложение вида *"You are assistant."*. Посмотреть применяемый в локальной LLM *системный запрос* можно, либо на сайте [*hf.co*](https://huggingface.co/), либо в применяемой программе.

Поэтому, если хочется, чтобы ответы от локальной LLM были столь же качественными, как и ответы от коммерческих сервисов, надо не лениться, и писать хорошие запросы, а ещё лучше, добавлять в запрос "шапку", которая будет объяснять модели _как ей следует думать_. Примеры хорошо структурированных запросов (PROMPT) можно посмотреть в библиотеках запросов, например <https://github.com/0xeb/TheBigPromptLibrary> или <https://github.com/abilzerian/LLM-Prompt-Library>.
...
Отдельно можно выделить модели дообученные на больших массивах программного кода с упором на логичность, точность и скурпулёзность до самого последнего символа, в ущерб "человечности" диалога и прочих социальных штучек. Такие модели называются coder, coding и т.п. Основная область применения этих моделей, это в составе средств разработки, для задач автокомплита, написания кода, тестирования, поиска ошибок в коде, поиска логических ошибок в текстах и формулах и т.п. Из-за того, что для продуктивной работы требуется хорошая отзывчивость среды разработки, потребуется большая скорость выполнения LLM, для чего нужны очень мощные вычислительные ресурсы, которые недоступны большинству пользователей; поэтому, скорее всего, эти модели придётся использовать по платной подписке не смотря на их открытость. Coder модели, это _Non-Reasoning_ модели, заточенные, в первую очередь на точность и скорость; они, конечно, могут написать простую программу по запросу, но если потребуется создание чего-то со сложной математикой или алгоритмами, то coder модели однозначно уступят любой сравнимой по размеру _Reasoning_ модели.

_Резюмируя:_

* Модели base нужны _только_ для исследователей и создателей LLM для дальнейшего дообучения для каких-то частных конкретных задач;
* Instruct-модели умеют вести диалог, все онлайн-сервисы --- это instruct-модели;
* Instruct-модели делятся на на _Reasoning_, и _Non-Reasoning_. Первые лучше отвечают, но намного медленнее вторых;
* Coder модели для программирования, могут применяться как в средах разработки, так и писать несложный код по запросу;


Технологии LLM продолжают постоянно улучшаться, и самые современные instruct модели (2025) дообучаются методом *Reinforcement Learning* (RL) --- [Обучение с подкреплением](https://ru.wikipedia.org/wiki/Обучение_с_подкреплением), в отличии от предыдущего поколения LLM, которые обучались методом *Reinforcement Learning from Human Feedback* (RLHF) --- [Обучение с подкреплением на основе отзывов](https://ru.wikipedia.org/wiki/Обучение_с_подкреплением_на_основе_отзывов_людей). Т.е. при обучении, помимо заранее заданных массивов вопрос-ответ, или массивов логических цепочек, модель сама в процессе обучения создаёт новые пары вопрос-ответ, новые логические цепочки, и проверяет их на корректность, либо самостоятельно, либо с помощью другой языковой модели; результаты этой проверки становятся ключом для дальнейшего подкрепления. Этот метод обучения приводит к некой _Самоинтроспекции_ (_Self-Reasoning_) моделей, в результате чего внутренние данные моделей становятся лучше взаимосвязаны, кластеризованы, лучше ассоциированы; такие модели меньше ошибаются, лучше улавливают контекст запросов. Обученные методом *RL* модели могут быть как _Reasoning_, так и _Non-Reasoning_.


Дальнейший прогресс обучения методом *Reinforcement Learning* привёл к появлению моделей *"Mixture of Experts"* (MoE), или *"комбинация экспертов"* (*"смесь экспертов"*), в которых в каждый конкретный момент работы, задействуется только небольшая часть (типично 10%) весов, непосредственно ответственная за решение задачм. MoE модели можно представить как набор небольших специализированных моделей в общей упаковке, с маршрутизатором отправляющим данные тому или иному "эксперту" в зависимости от содержимого данных. Такие *MoE* модели имеют преимущество перед обычными "плотными" моделями в том, что требуют на порядок меньшего количества вычислительных ресурсов на выполнение при равных размерах. Из недостатков следует отметить, что общая точность несколько падает, и соответствует, примерно, в два раза меньшим, по размеру, "плотными" моделями.

_Резюмируя:_

* MoE модели будут в разы быстрее сравнимых по качеству "плотных" моделей.
...
Режим работы _Reasoning / Non-Reasoning_ выбирается добавлением в конец запроса ключа `/no_think` для режима без размышлений. Для примера, выполнение запроса к LLM из командной строки:
...
Несколько приближённо, но можно сказать, что каждый слой НС представляет собой матрицу, и прохождение информации через слой является операцией умножения этой матрицы на вектор, содержащий закодированную информацию. Вот ориентировочные размеры матриц для ключевых слоёв в типичной Transformer-модели (информация создана LLM Qwen):
...
2. Self-Attention (Q, K, V проекции)\
   Каждый из трёх проекторов (Query, Key, Value):
    - Матрицы: `(d_model × d_k)` или `(d_model × d_v)`
    - Обычно `d_k = d_v = d_model // num_heads`

    > Пример: GPT-2 small имеет `d_model=768`, `num_heads=12` → `d_k = d_v = 64`
    Тогда каждая матрица Q/K/V: `768 × 64`
    Иногда эти три матрицы объединяются в одну: `d_model × (3*d_k)`

3. Проекция после Self-Attention (Dense)\
   После конкатенации голов внимания:
    - Матрица: `(d_model × d_model)`

4. Feed-Forward Network (FFN)\
   Обычно состоит из двух линейных слоёв:
    - Первый: `(d_model × d_ff)`
    - Второй: `(d_ff × d_model)`
      - `d_ff`: обычно в 2–8 раз больше `d_model` (например, 2048–30720)

    > Пример: Для GPT-2 small `d_model=768`, `d_ff=3072`
    → Матрицы: `768×3072` и `3072×768`
...
: Примеры приблизительных размеров матриц для популярных моделей

_Note:_ Размеры матриц растут квадратично с увеличением `d_model`.



## LLM chain {.unnumbered .unlisted}
