...
* несколько приближённо, но можно сказать, что каждый слой НС представляет собой матрицу^[про матрицы подробнее в [описании алгоритма](#алгоритм-работы-нейросети-трансформера)], и прохождение информации через слой является операцией умножения этой матрицы на вектора (эмбеддинги), содержащие закодированную информацию;
...
_Note:_ Можно писать запросы в одну строку, в нужных местах вставляя символы `\n` для переноса строки для форматирования; это может потребоваться при работе через программные интерфейсы или CLI, когда модели передаётся одна строка текста.
