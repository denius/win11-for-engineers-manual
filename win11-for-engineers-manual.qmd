...
Скорее всего в ближайшее время все коммерческие большие языковые модели перейдут на [*Системные Запросы*](#отступление-об-особенностях-запросов-для-локальных-llm) (*System Prompt*) в какой-то похожей форме; и, даже вполне возможно, что кто-то уже сейчас применяет схожие технологии.
...
Добавить ИИ помощник в произвольный редактор достаточно сложно, так как такой помощник меняет логику применения редактора. Настройка помощника тоже не простая. И тут может оказаться, что написать новый редактор заточенный на применение с ИИ может быть проще, чем модифицировать существующие. Итак, сначала будет рассмотрен ИИ редактор [zed](#zed), а далее несколько расширений для поддержки ИИ в уже существующих редакторах.
