...
Большие языковые модели (Large Language Models, LLM), это искусственные нейронные сети (НС) типа [трансформер](https://ru.wikipedia.org/wiki/Большая_языковая_модель#Transformer); они имеют следующие особенности:
...
* состоит из слоёв, типичное число слоёв 20-60;
* типичные размеры входа --- тысячи и десятки тысяч токенов; выход, как правило, на порядок больше входа;


Примерный алгоритм работы нейросети-трансформера [приведён в конце главы](#алгоритм-работы-нейросети-трансформера).


Языковые модели LLM бывают закрытые, и с открытыми весами. В отличии от закрытых, модели с открытыми весами могут быть загружены с инета и запущены локально на своих вычислительных ресурсах. Это единственное их различие, в остальном они одинаковы в плане доступа: все они доступны онлайн, есть модели с платным доступом, есть с бесплатным, но большинство моделей имеют смешанный тип доступа.
...
За один запрос модель вряд ли сможет выполнить сложную задачу, поэтому задачу необходимо разбить на этапы, как в примере со справочником. Для того, чтобы модель за раз смогла выполнить сложную задачу, необходимо составлять сложные, многоэтапные запросы, с примерами, с критериями, и т.п. (см. далее).

Даже для несложных запросов ответы модели значительно улучшаются, если запрос должным образом структурирован. Ещё немаловажный плюс от структурирования запросов в том, что пользователь сам начинает лучше понимать постановку задачи. Пример структурированного запроса:
...
Реально универсальные модели имеют размер более сотни миллиардов весов (100b), что делает невозможным их локальный запуск. Модели доступные для локального запуска имеют те или иные сильные и слабые стороны; и выбор подходящей специализированной модели порой имеет большее влияние на успешность решения задачи, чем выбор модели большего размера.
...
* Со сложными задачами могут справится только _Reasoning_ модели, например [Qwen3-30B-A3B](https://ollama.com/library/qwen3) или [DeepSeek R1](https://ollama.com/library/deepseek-r1).
...
* _Reasoning_ модели --- модели с размышлениями, которые перед ответом, проговаривают все аспекты, которые возможно потребуются в ответе, на основе которых потом строится полный ответ. При равных размерах, и/или степени квантизации, модели с размышлениями всегда сильнее простых моделей.
...
7. Для сопоставления выходному эмбеддингу конкретного токена, этот выходной эмбеддинг проходит через дополнительный _линейный слой_ НС, который преобразует его в вектор^[называется logits] размером со словарь токенов (см. п.1), а затем через функцию softmax --- в вероятностное распределение для каждого токена из словаря. Следующий токен ответа выбирается согласно этому распределению в зависимости от заданного алгоритма: жадный (greedy) — самый вероятный; случайный выбор среди наиболее вероятных с учётом температуры; top-k sampling, или nucleus sampling (top-p) и др.
