...
В 2025г начали развиваться методологии формирования запросов с целью создания ~~Операционной Системы~~ Контекстно Ориентированных Систем на основе языковых моделей LLM. Данный подход предполагает создание запроса специального вида, в процессе выполнения которого образуется цепочка размышлений, формирующая цикл размышления в LLM модели, который, в свою очередь, образует граф состояний (граф размышлений), с возможностью возвращаться обратно по цепочкам графа, или двигаться по циклу графа размышлений вперёд с возможностью модификации состояний (текста/эмбеддингов). Данный подход позволяет с лёгкостью выполнять самоинтроспекцию (саморизонинг), итеративное самоулучшение модели в виде модификации запроса и повторного выполнения обновлённого запроса в рамках последующего цикла.

Такой подход позволяет формировать в языковой модели некоторую "среду размышлений", в рамках которой пользователь может углубляться в детали, выходить на верхний уровень графа размышлений, что-то изучать подробнее, прояснять и так далее --- иными словами, проводить работу в рамках заданной темы в контексте уже заданных вопросов и уже полученных ответов.

Один из примеров создания подобных систем демонстрируется в публикации [Промт для изучения чего угодно: протокол Олега-Деминга](https://habr.com/ru/companies/bar/articles/933248/), а сами запросы представлены в двух вариантах: [стандартный](https://gitverse.ru/anarchic/oleg-deming-protocol/content/main/OLEG-DEMING-PROTOCOL-STANDARD.md) и [въедливый](https://gitverse.ru/anarchic/oleg-deming-protocol/content/main/OLEG-DEMING-PROTOCOL-ROLEPLAY.md). Как видно из текстов запросов это весьма сложные коды, которые содержат переменные, циклы и другие техники программирования, которые никак не ожидаешь увидеть в запросах. _Note:_ Из сложности этих запросов следует ожидать, что их смогут выполнить только достаточно большие Reasoning LLM, а доступные локальные модели с запросами такой сложности не справятся.

Скорее всего в ближайшее время все коммерческие большие языковые модели перейдут на *Системные Запросы* (*System Prompt*) в какой-то похожей форме, а возможно какие-то коммерческие системы уже перешли.
