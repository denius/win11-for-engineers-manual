...
Несколько более подробно о степенях квантизации в главе [*Квантизация моделей*](#квантизация-моделей).
...
5. Этот процесс повторяется несколько раз: последовательность эмбеддингов проходит через множество уровней (называется _блоком_), состоящих из пары слоёв: self-attention (п.3) + FFN (п.4). Обычно таких уровней от 12 до 48. На каждом уровне модель углубляет понимание контекста и взаимосвязей между элементами последовательности.
...
Число после `Q` означает, что веса модели были округлены, *в среднем*, до **_x_** бит на один коэффициент, причём веса после квантизации хранятся в виде _целочисленных_ (*Integer*) значений соответствующей длины.
...
`Q8_0` означает, что _все_ коэффициенты в весах модели были квантованы до 8 бит на вес. Аналогично, `Q4_0` --- квантованы до 4 бит на вес.

Суффиксы `_S`, `_M`, `_L` в `Qx_K_S`, `Qx_K_M`, `Qx_K_L` означают, что тензоры *`ffn`* были округлены, в среднем, до **_x_** бит на один коэффициент, а более важные тензоры внимания *`attn`* будут квантованы на 1, 2 или 3 бит точнее, чем **_x_**, например, `Q3_K_S` означает 3 бита для *`ffn`* и 4 бита для *`attn`*.

[Динамическая квантизация](https://docs.unsloth.ai/basics/unsloth-dynamic-2.0-ggufs) `_XL` в `Qx_K_XL` означает, что важные тензоры внимания *`attn`* оставлены практически в исходном качестве, причём, для разных блоков (слоёв) это качество может ещё и варьироваться от блока к блоку, что подбирается непосредственно для каждой модели.

Квантизация `IQx_` проводится с использованием матриц важности (*imatrix*), которые составляются из ключевых фраз, участков кода, в общем той текстовой информации, которую требуется сохранить в модели при квантизации. Те блоки тензоров, что слабо реагируют на матрицу важности округляются сильнее, чем блоки, которые "замечают" _imatrix_. Такой способ квантизации позволяет в значительной степени управлять информацией, которая остаётся в квантованной модели.

Ещё существует перспективная динамическая квантизация `_R4` для больших степеней квантования, но только в виде одного инструмента [ik_llama.cpp](https://github.com/ikawrakow/ik_llama.cpp?tab=readme-ov-file#iqk-quants).
