...
format:
  pdf:
    ...
    monofont: UbuntuCondensed Nerd Font # All fitted!
    # monofont: D2CodingLigature Nerd Font #tilde_auto
    # monofont: IosevkaTerm NF #tilde_aut
    # monofont: ZedMono Nerd Font Mono #tilde_aut
    # monofont: M+1Code Nerd Font Mono #tilde_aut
    # monofont: Monoid Nerd Font Mono #tilde
    # monofont: Sudo #tilde
    # monofont: InputMonoCondensed Nerd Font #ti
...
_Disclaimer:_ В последние годы стало возможным приобрести недорогие OEM лицензии на продукты Microsoft, на Озоне или Вайлбериз, поэтому у пользователей появился выбор между применением KMS сервисов, и установкой OEM лицензий. Из минусов установки OEM лицензий --- потребуется создание учётной записи на сайте Microsoft для онлайн регистрации продуктов --- регистрация оффлайн стала практически невозможна. Ну, и, OEM лицензии необходимо покупать только у проверенных продавцов со множеством отзывов, но это обычные издержки электронной торговли.


_Note:_ Лицензии OEM --- это лицензии на ПО, поставляемые производителями и сборщиками техники вместе с новыми ноутбуками и компьютерами. Microsoft выделяет производителям техники целые пулы OEM лицензий по небольшой цене. Но, по Европейским законам, производители могут продавать не только продукт целиком, но и комплектующие к нему, и, в данном случае, они могут продавать эти лицензии отдельно от компьютерной техники. Каждая OEM лицензия входит в какой-то конкретный пул лицензий, который закреплён за конкретным производителем, т.е., OEM, это не какая-то абстрактная безликая лицензия, а выпущенная определённым производителем. В целом, покупка OEM лицензии достаточно безопасна, случаев отзыва пулов лицензий очень и очень немного. Из особенностей OEM лицензии следует отметить, что она, в отличии от Retail или Volume лицензий, "одноразовая", то есть, активировав её один раз, _ключ_ нельзя применить второй раз, и программу нельзя будет переустановить или перенести на другой компьютер.

_Note:_ KMS активация. Volume (корпоративные) лицензии можно активировать с помощью KMS сервисов по сети. В интернете есть несколько компаний, бесплатно предоставляющих такие лицензии; их можно использовать, указав в качестве адреса сервера активации сайты этих компаний. Конечно, в случае прекращения работы такого сайта, активация программы останавливается. Активации с помощью KMS сервиса по локальной сети, это совершенно стандартная процедура, которая происходит автоматически для продуктов с Volume лицензией при правильной настройке свойств локального сетевого подключения (необходимо указать в качестве DNS сервера адрес KMS сервиса). В случае подсоединения через интернет, возможность KMS активации ограничена, и, регистрировать онлайн KMS сервис необходимо с помощью специальных утилит.


## Скачать официальные установщики пакетов Windows и MSOffice

На данный момент все инсталяторы для [продуктов Microsoft загружаются через интернет](https://www.microsoft.com/en-us/download), это официальный рекомендуемый способ установки --- диски отошли в прошлое. [Windows](https://www.microsoft.com/en-us/download/windows), [Office](https://www.microsoft.com/en-us/download/office).

> В данном руководстве приходится расписывать процедуры загрузки продуктов Microsoft из-за санкций, которые ограничивают загрузку напрямую, непосредственно с сайта Microsoft (хотя никакого запрета на продажу или использование лицензий Microsoft не существует).\
  \
  См. главу [UUP — Unified Update Platform — загрузка MS Windows](#uup-unified-update-platform-загрузка-ms-windows).\
  \
  См. главу [ODT — Office Deployment Tool — загрузка MS Office](#odt-office-deployment-tool-загрузка-ms-office).

## Выбор версии Windows

_Note:_ Поддержка Windows 10 закончится осенью 2025, после чего, скорее всего, интернет заполонят вирусы для этой версии Windows из-за отсутствия обновлений безопасности. Поэтому рекомендуется изначально устанавливать Windows 11.

Windows 11 устанавливать следует _En_ версию Windows 11 Pro. [Скачать](https://uupdump.net/download.php?id=07dd20c8-a2de-4792-a336-2ab01b43a160&pack=en-us&edition=professional) установочный ~~диск~~ скрипт для создания iso-диска версии _build 22631.3296_ (_23H2_) с официального сайта <https://uupdump.net/> --- Unified Update Platform (UUP) (подробности в [Приложении](#uup-unified-update-platform-загрузка-ms-windows)). Это версия, с одной стороны, достаточно свежая, чтобы были драйвера для новых ноутов, а с другой стороны, эта версия всё ещё позволяет при установке создать локальную учётную запись без необходимости онлайн регистрации учётной записи Microsoft (можно использовать и более новые версии при настройке с помощью Rufus, см. далее), а также не пытается удалить КМС при активации.
...
*\<TBD\>* Чем новее версия, тем лучше.

## Установка с лицензией

Загрузиться с загрузочной флешки с инсталятором Windows. Для установки потребуется подключение к интернет для входа в учётную запись Microsoft (или создания новой).

Запустить установщик MSOffice. После установки открыть любое приложение MSOffice, Word например; снизу слева будет меню _Учетная запись_, в нём выбрать _Активировать..._.


## Установка с KMS активацией

При установке Windows не подключать интернет! --- сказать, что нет интернета. И, соответственно, установка будет идти без учётной записи Microsoft --- будет только локальное имя и пароль. Логин пользователя должен быть из латинских букв и без пробелов, чтобы избежать возможных проблем со старыми программами, которые не понимают юникод. Регион выбрать _US_, и вообще всё _En_, можно только добавить клавиатуру _Ru_, но не делать её первой. После установки все эти настройки можно будет сменить, и даже полностью сменить интерфейс на русский.
...
Сразу после активации из этого же запущенного KMS можно установить MSOffice --- там есть кнопка. Выбирать канал _Current_ (справа), и версию 2024ProPlus или 2021ProPlus. После установки опять нажать Активация Оффлайн, и там внизу выбрать из выпадающего списка Office/Visio/ProjectPro и каждого из них активировать. Этот вариант установки, возможно, позволит избежать возни с восстановлением файлов. Но, у этой КМС-утилиты есть недостаток --- она сбрасывает канал загрузки офиса, и, если не уследить, то офис будет без апдейта функционала, а будут только обновления безопасности. Поэтому, возможно, стоит воспользоваться следующим вариантом установки офиса.

MSOffice лучше попробовать установить утилитой _"Office 2013-2024 C2R Install + Lite v7.7.7.7 ++ r25"_ --- она не сбрасывает канал (_Current_) при выборе версии (_2024ProPlus, ru_ru_) офиса. Но возможно потребуется восстановить файлы удалённые Windows Defender'ом --- если он уже успел обновиться, то будет удалять файлы КМС-утилиты.
...
* Отключить режим гибернации (hibernate): в PowerShell(Admin) выполнить:\
  `powercfg /H off`
...
* Включить встроенный многокарманный буфер обмена (Clipboard) в Windows --- `Win+V` (_Note:_ этот буфер очищается при перезагрузке!, поэтому лучше использовать сторонний буфер).
...
* Если Edge не используется, то есть пара утилит на случай, если Edge станет совсем занудным своими напоминаниями: [GoAwayEdge](https://github.com/valnoxy/GoAwayEdge) и [MSEdgeRedirect](https://github.com/rcmaehl/MSEdgeRedirect), которые отключает Edge и регулярно проверяет, чтобы Edge больше не просыпался.
...
_Note:_ [Руководство](https://www.geeksforgeeks.org/enable-and-disable-keyboard-shortcuts/), как включить или выключить те или иные комбинаций клавиш быстрого доступа Windows.
...
Все консольные программы надо устанавливать через _Scoop_, при этом программы будут установлены в домашнюю папку, и пути будут добавлены в переменную _PATH_, что автоматически позволит запускать эти программы из командной строки.

Графические программы следует устанавливать либо через _WinGet_, либо через _Chocolatey_ --- примерно одно и тоже. Через _winget_ стоит устанавливать те программы, которым не требуется регулярное обновление, либо которые умеют сами обновляться, например Telegram --- установил и забыл. Если программы требуют регулярного контроля над обновлениями, то это удобнее делать через _ChocolateyGUI._
...
scoop checkup
```
Здесь потребуется пара настроек Windows для _Developer Mode_ и _LongPaths_, если они не были сделаны при установке.
...
_Note:_ В консольных терминалах для выполнения программ под правами _Admin_ следует вставлять _sudo_ перед командой, например: `sudo choco install -y vcredist-all`.
...
Несколько слов о разных программах и утилитах, которые не требуют подробного описания установки или использования. Большинство из них были в списках установки выше, т.е. _уже_ были установлены.


### Программы для картинок

[**nomacs**](<https://nomacs.org/>) --- программа для просмотра изображений; а также для их модификации, с широчайшими возможностями: конвертация, масштабирование, вращение, обрезка, работа с цветом, баланс белого, работа с raw форматами, интерактивная обработка синхронно нескольких изображений. Конечно, не фотошоп, но для обычного пользователя 99% необходимого функционала обеспечивает.
...
Если бы не было просмотра pdf в каждом браузере, то можно было бы установить [Sumatra PDF](https://www.sumatrapdfreader.org/free-pdf-reader):
```cmd
...
Офисные программы рассмотрены в отдельной [главе](#офисные-пакеты). Написание программных кодов предполагается в _VSCode_, настройка которого рассмотрена в отдельной главе.

* [**Notepad++**](https://notepad-plus-plus.org/) --- легковесная замена блокнота, ультраэдита, и проч., с подсветкой синтаксиса, широкими возможностями редактирования. Устанавливается: `scoop install extras/notepadplusplus`.

* [**Meld**](https://meldmerge.org/) и [**WinMerge**](https://winmerge.org/) --- это программы для сравнения и редактирования файлов бок о бок, наиболее популярные из открытых. Используются, в том числе, для разрешения конфликтов при слияниях *git*. Устанавливаются, соответственно: `winget install --id=Meld.Meld -e` и `scoop install extras/winmerge`.

* [**GoldenDict-ng**](https://xiaoyifang.github.io/goldendict-ng/) --- локальные словари для перевода. По умолчанию активируется (переводит) по двойному нажатию на `<Ctrl+C>`. [Устанавливается](https://winstall.app/apps/xiaoyifang.GoldenDict-ng) так: `winget install --id=xiaoyifang.GoldenDict-ng -e`; но при установке никаких словарей не ставится, их необходимо дополнительно скачать и указать программе где они, скачанные, лежат.
  Свободные словари: *\<TBD\>*
...
* [**WinDirStat**](https://windirstat.net/) --- утилита для контроля занимаемого места файлами на диске, показывает размер файлов в виде плиток. Установка: `winget install --id=WinDirStat.WinDirStat -e`
...
[**HW-Info**](https://www.hwinfo.com/) --- бесплатная проприетарная утилита, выводит список наличных аппаратных устройств:
```cmd
...
Установить _FirefoxESR_: `choco install -y firefoxesr`, установить _МастерПароль_, установить [_NoScript_](https://addons.mozilla.org/en-US/firefox/addon/noscript/), [_U-Block Origin_](https://addons.mozilla.org/en-US/firefox/addon/ublock-origin/). Этот браузер будет для работы с конфиденциальными данными, для работы: с почтой, банковскими сайтами, и т.п. Поначалу придётся много раз добавлять сайты в разрешённые в _NoScript_, но их всё же ограниченное число.
...
_Note:_ Поменять шрифты можно кликнув правой кнопкой мыши на иконке Far, в свойствах.
...
Запускать [**Far**](https://www.farmanager.com/) в [**WindowsTerminal**](https://learn.microsoft.com/ru-ru/windows/terminal/), в котором можно настроить цвета/палитры, шрифты и т.п.
...
* Тема оформления, Шрифт, Форма курсора, меняются в настройках WindowsTerminal в _Settings > Defaults > Appearance_ -- это будут настройки по умолчанию для всех новых типов терминалов. Эти настройки можно будет изменить для каждого профиля терминала отдельно.
...
В [приложении показана настройка Clink](#консольная-оболочка-clink-и-утилиты-busybox).
...
Для создания ssh-соединения по _OpenSSH_ выполнить в _cmd_/_Clink_/_Far_:
```cmd
set DISPLAY=127.0.0.1:0.0
ssh -X somename@somehost
```
тогда проброс (Forwarding) протокола *X11*  заработает ([guide](https://x410.dev/cookbook/built-in-ssh-x11-forwarding-in-powershell-or-windows-command-prompt/)).
...
Поэтому, либо скачать предыдущую версию 2.5.xy (2.5.10) с сайта <https://openvpn.net/community-downloads/> и после установки зафиксировать эту версию:
```cmd
winget pin add OpenVPNTechnologies.OpenVPN
```

Либо ~~отредактировать _.ovpn_ файл~~ скачать ключики заново -- они будут уже в новом формате.
...
* так же эта технология позволяет настроить Continuous Integration (CI) --- сервис Непрерывной Интеграции, запускающий набор тестов для каждого изменения/commit внесённого в исходный код.
...
Для просмотра векторов в окошке _Watch_ нажать _+_ и набрать `argv, 4`, где _4_ это длина просматриваемого участка вектора.

## Python

На Windows можно установить нативный компилятор Python и его библиотеки, без использования WSL.

У Python есть несколько менеджеров окружений, которые используются для разделения установленных пакетов на независимые "песочницы", с тем, чтобы было возможно установить пакеты, которые имеют зависимости, конфликтующие с зависимостями других пакетов. Помимо встроенного в Python менеджера [**pip**](https://pypi.org/project/pip/) существует простой и удобный [**pipx**](https://pipx.pypa.io/stable/). На практике, при установке широкоизвестных библиотек лучше использовать стандартный метод `pip install --upgrade some-python-library`. Но если устанавливается Python-программа, особенно с большим числом зависимостей, то её лучше установить в отдельное окружение: `pipx install some-python-app`.
...
_Note:_ после установки Python, чтобы python-программы с расширением _.py_ можно было запускать без добавки расширения, необходимо, чтобы расширение _.py_ [присутствовало](https://docs.python.org/3/faq/windows.html#id3) в переменной окружения _PATHEXT_.
...
В [**Spyder**](https://docs.spyder-ide.org/current/quickstart.html) всё настроено для Python "из коробки". Основной упор сделан на DataScience: обработка данных на Python, построение графиков, статистика. По стилю работы похож на MatLab. При первом запуске проводится краткий обучающий тур основам использования.
```cmd
...
Так же для _DataScience_ стоит попробовать [**Positron**](https://positron.posit.co/start.html). Это VSCode, где встроили набор плагинов для обработки данных на Python/R и проч. По стилю работы очень похож на VSCode (что подкупает ненужностью изучения чего-то непохожего), но это пока проект в beta стадии, могут встречаться ошибки.
```cmd
python -m pip install --upgrade ipykernel
scoop install extras/positron
```

### Python в PyCharm {.unnumbered .unlisted}

Для профессиональной разработки на Python возможно стоит рассмотреть _проприетарный_ PyCharm --- порог вхождения выше, но и возможностей больше.
```cmd
...
Создайте файл _tasks.json_ для настройки компиляции: открыть командную палитру `<Ctrl+Shift+P>`, ввести _Tasks: Configure Task_, выбрать _Create tasks.json file from template_, выбрать _Others_, отредактировать на:
```json
{
...
Пользователи Vim могут воспользоваться [расширением VSCode Neovim](https://marketplace.visualstudio.com/items?itemName=asvetliakov.vscode-neovim), который запускает полноценную версию Neovim в редакторе VSCode. Будут доступны обычный _init.lua_ и [большинство](https://github.com/vscode-neovim/vscode-neovim/wiki/Plugins) привычных Vim плагинов.
...
[**tldt**](https://github.com/tldr-pages/tldr) -- <https://tldr.sh/>.
...
[Установка](https://scoop.sh/#/apps?o=true&q=tldr):
```cmd
scoop install tlrc
...
С такой конфигурацией, если в шаблоне нет заглавных букв, то `rg` будет считать шаблон _ignorecase_, иначе шаблон _casesensitive_. Строчка конфигурации _\-\-auto-hybrid-regex_ указывает применять разные библиотеки _regex_ в зависимости от сложности шаблона --- для простых шаблонов это значительно ускоряет поиск.
...
```cmd
doskey time=%UserProfile%\scoop\shims\sort.exe $*
...
[_\[_](https://www.busybox.net/downloads/BusyBox.html#test)
[_\[\[_](https://www.busybox.net/downloads/BusyBox.html#test)
[_acpid_](https://www.busybox.net/downloads/BusyBox.html#acpid)
[_addgroup_](https://www.busybox.net/downloads/BusyBox.html#addgroup)
[_adduser_](https://www.busybox.net/downloads/BusyBox.html#adduser)
[_adjtimex_](https://www.busybox.net/downloads/BusyBox.html#adjtimex)
[_ar_](https://www.busybox.net/downloads/BusyBox.html#ar)
[_arp_](https://www.busybox.net/downloads/BusyBox.html#arp)
[_arping_](https://www.busybox.net/downloads/BusyBox.html#arping)
[_ash_](https://www.busybox.net/downloads/BusyBox.html#ash)
[_awk_](https://www.busybox.net/downloads/BusyBox.html#awk)
[_basename_](https://www.busybox.net/downloads/BusyBox.html#basename)
[_beep_](https://www.busybox.net/downloads/BusyBox.html#beep)
[_blkid_](https://www.busybox.net/downloads/BusyBox.html#blkid)
[_brctl_](https://www.busybox.net/downloads/BusyBox.html#brctl)
[_bunzip2_](https://www.busybox.net/downloads/BusyBox.html#bunzip2)
[_bzcat_](https://www.busybox.net/downloads/BusyBox.html#bzcat)
[_bzip2_](https://www.busybox.net/downloads/BusyBox.html#bzip2)
[_cal_](https://www.busybox.net/downloads/BusyBox.html#cal)
[_cat_](https://www.busybox.net/downloads/BusyBox.html#cat)
[_catv_](https://www.busybox.net/downloads/BusyBox.html#catv)
[_chat_](https://www.busybox.net/downloads/BusyBox.html#chat)
[_chattr_](https://www.busybox.net/downloads/BusyBox.html#chattr)
[_chgrp_](https://www.busybox.net/downloads/BusyBox.html#chgrp)
[_chmod_](https://www.busybox.net/downloads/BusyBox.html#chmod)
[_chown_](https://www.busybox.net/downloads/BusyBox.html#chown)
[_chpasswd_](https://www.busybox.net/downloads/BusyBox.html#chpasswd)
[_chpst_](https://www.busybox.net/downloads/BusyBox.html#chpst)
[_chroot_](https://www.busybox.net/downloads/BusyBox.html#chroot)
[_chrt_](https://www.busybox.net/downloads/BusyBox.html#chrt)
[_chvt_](https://www.busybox.net/downloads/BusyBox.html#chvt)
[_cksum_](https://www.busybox.net/downloads/BusyBox.html#cksum)
[_clear_](https://www.busybox.net/downloads/BusyBox.html#clear)
[_cmp_](https://www.busybox.net/downloads/BusyBox.html#cmp)
[_comm_](https://www.busybox.net/downloads/BusyBox.html#comm)
[_cp_](https://www.busybox.net/downloads/BusyBox.html#cp)
[_cpio_](https://www.busybox.net/downloads/BusyBox.html#cpio)
[_crond_](https://www.busybox.net/downloads/BusyBox.html#crond)
[_crontab_](https://www.busybox.net/downloads/BusyBox.html#crontab)
[_cryptpw_](https://www.busybox.net/downloads/BusyBox.html#cryptpw)
[_cut_](https://www.busybox.net/downloads/BusyBox.html#cut)
[_date_](https://www.busybox.net/downloads/BusyBox.html#date)
[_dc_](https://www.busybox.net/downloads/BusyBox.html#dc)
[_dd_](https://www.busybox.net/downloads/BusyBox.html#dd)
[_deallocvt_](https://www.busybox.net/downloads/BusyBox.html#deallocvt)
[_delgroup_](https://www.busybox.net/downloads/BusyBox.html#delgroup)
[_deluser_](https://www.busybox.net/downloads/BusyBox.html#deluser)
[_depmod_](https://www.busybox.net/downloads/BusyBox.html#depmod)
[_devmem_](https://www.busybox.net/downloads/BusyBox.html#devmem)
[_df_](https://www.busybox.net/downloads/BusyBox.html#df)
[_dhcprelay_](https://www.busybox.net/downloads/BusyBox.html#dhcprelay)
[_diff_](https://www.busybox.net/downloads/BusyBox.html#diff)
[_dirname_](https://www.busybox.net/downloads/BusyBox.html#dirname)
[_dmesg_](https://www.busybox.net/downloads/BusyBox.html#dmesg)
[_dnsd_](https://www.busybox.net/downloads/BusyBox.html#dnsd)
[_dnsdomainname_](https://www.busybox.net/downloads/BusyBox.html#dnsdomainname)
[_dos2unix_](https://www.busybox.net/downloads/BusyBox.html#dos2unix)
[_dpkg_](https://www.busybox.net/downloads/BusyBox.html#dpkg)
[_du_](https://www.busybox.net/downloads/BusyBox.html#du)
[_dumpkmap_](https://www.busybox.net/downloads/BusyBox.html#dumpkmap)
[_dumpleases_](https://www.busybox.net/downloads/BusyBox.html#dumpleases)
[_echo_](https://www.busybox.net/downloads/BusyBox.html#echo)
[_ed_](https://www.busybox.net/downloads/BusyBox.html#ed)
[_egrep_](https://www.busybox.net/downloads/BusyBox.html#egrep)
[_eject_](https://www.busybox.net/downloads/BusyBox.html#eject)
[_env_](https://www.busybox.net/downloads/BusyBox.html#env)
[_envdir_](https://www.busybox.net/downloads/BusyBox.html#envdir)
[_envuidgid_](https://www.busybox.net/downloads/BusyBox.html#envuidgid)
[_expand_](https://www.busybox.net/downloads/BusyBox.html#expand)
[_expr_](https://www.busybox.net/downloads/BusyBox.html#expr)
[_fakeidentd_](https://www.busybox.net/downloads/BusyBox.html#fakeidentd)
[_false_](https://www.busybox.net/downloads/BusyBox.html#false)
[_fbset_](https://www.busybox.net/downloads/BusyBox.html#fbset)
[_fbsplash_](https://www.busybox.net/downloads/BusyBox.html#fbsplash)
[_fdflush_](https://www.busybox.net/downloads/BusyBox.html#fdflush)
[_fdformat_](https://www.busybox.net/downloads/BusyBox.html#fdformat)
[_fdisk_](https://www.busybox.net/downloads/BusyBox.html#fdisk)
[_fgrep_](https://www.busybox.net/downloads/BusyBox.html#fgrep)
[_find_](https://www.busybox.net/downloads/BusyBox.html#find)
[_findfs_](https://www.busybox.net/downloads/BusyBox.html#findfs)
[_flash_lock_](https://www.busybox.net/downloads/BusyBox.html#flash_lock)
[_flash_unlock_](https://www.busybox.net/downloads/BusyBox.html#flash_unlock)
[_fold_](https://www.busybox.net/downloads/BusyBox.html#fold)
[_free_](https://www.busybox.net/downloads/BusyBox.html#free)
[_freeramdisk_](https://www.busybox.net/downloads/BusyBox.html#freeramdisk)
[_fsck_](https://www.busybox.net/downloads/BusyBox.html#fsck)
[_fsck.minix_](https://www.busybox.net/downloads/BusyBox.html#fsck.minix)
[_fsync_](https://www.busybox.net/downloads/BusyBox.html#fsync)
[_ftpd_](https://www.busybox.net/downloads/BusyBox.html#ftpd)
[_ftpget_](https://www.busybox.net/downloads/BusyBox.html#ftpget)
[_ftpput_](https://www.busybox.net/downloads/BusyBox.html#ftpput)
[_fuser_](https://www.busybox.net/downloads/BusyBox.html#fuser)
[_getopt_](https://www.busybox.net/downloads/BusyBox.html#getopt)
[_getty_](https://www.busybox.net/downloads/BusyBox.html#getty)
[_grep_](https://www.busybox.net/downloads/BusyBox.html#grep)
[_gunzip_](https://www.busybox.net/downloads/BusyBox.html#gunzip)
[_gzip_](https://www.busybox.net/downloads/BusyBox.html#gzip)
[_hd_](https://www.busybox.net/downloads/BusyBox.html#hd)
[_hdparm_](https://www.busybox.net/downloads/BusyBox.html#hdparm)
[_head_](https://www.busybox.net/downloads/BusyBox.html#head)
[_hexdump_](https://www.busybox.net/downloads/BusyBox.html#hexdump)
[_hostid_](https://www.busybox.net/downloads/BusyBox.html#hostid)
[_hostname_](https://www.busybox.net/downloads/BusyBox.html#hostname)
[_httpd_](https://www.busybox.net/downloads/BusyBox.html#httpd)
[_hush_](https://www.busybox.net/downloads/BusyBox.html#hush)
[_hwclock_](https://www.busybox.net/downloads/BusyBox.html#hwclock)
[_id_](https://www.busybox.net/downloads/BusyBox.html#id)
[_ifconfig_](https://www.busybox.net/downloads/BusyBox.html#ifconfig)
[_ifdown_](https://www.busybox.net/downloads/BusyBox.html#ifdown)
[_ifenslave_](https://www.busybox.net/downloads/BusyBox.html#ifenslave)
[_ifplugd_](https://www.busybox.net/downloads/BusyBox.html#ifplugd)
[_ifup_](https://www.busybox.net/downloads/BusyBox.html#ifup)
[_inetd_](https://www.busybox.net/downloads/BusyBox.html#inetd)
[_init_](https://www.busybox.net/downloads/BusyBox.html#init)
[_inotifyd_](https://www.busybox.net/downloads/BusyBox.html#inotifyd)
[_insmod_](https://www.busybox.net/downloads/BusyBox.html#insmod)
[_install_](https://www.busybox.net/downloads/BusyBox.html#install)
[_ionice_](https://www.busybox.net/downloads/BusyBox.html#ionice)
[_ip_](https://www.busybox.net/downloads/BusyBox.html#ip)
[_ipaddr_](https://www.busybox.net/downloads/BusyBox.html#ipaddr)
[_ipcalc_](https://www.busybox.net/downloads/BusyBox.html#ipcalc)
[_ipcrm_](https://www.busybox.net/downloads/BusyBox.html#ipcrm)
[_ipcs_](https://www.busybox.net/downloads/BusyBox.html#ipcs)
[_iplink_](https://www.busybox.net/downloads/BusyBox.html#iplink)
[_iproute_](https://www.busybox.net/downloads/BusyBox.html#iproute)
[_iprule_](https://www.busybox.net/downloads/BusyBox.html#iprule)
[_iptunnel_](https://www.busybox.net/downloads/BusyBox.html#iptunnel)
[_kbd_mode_](https://www.busybox.net/downloads/BusyBox.html#kbd_mode)
[_kill_](https://www.busybox.net/downloads/BusyBox.html#kill)
[_killall_](https://www.busybox.net/downloads/BusyBox.html#killall)
[_killall5_](https://www.busybox.net/downloads/BusyBox.html#killall5)
[_klogd_](https://www.busybox.net/downloads/BusyBox.html#klogd)
[_last_](https://www.busybox.net/downloads/BusyBox.html#last)
[_length_](https://www.busybox.net/downloads/BusyBox.html#length)
[_less_](https://www.busybox.net/downloads/BusyBox.html#less)
[_linux32_](https://www.busybox.net/downloads/BusyBox.html#linux32)
[_linux64_](https://www.busybox.net/downloads/BusyBox.html#linux64)
[_linuxrc_](https://www.busybox.net/downloads/BusyBox.html#linuxrc)
[_ln_](https://www.busybox.net/downloads/BusyBox.html#ln)
[_loadfont_](https://www.busybox.net/downloads/BusyBox.html#loadfont)
[_loadkmap_](https://www.busybox.net/downloads/BusyBox.html#loadkmap)
[_logger_](https://www.busybox.net/downloads/BusyBox.html#logger)
[_login_](https://www.busybox.net/downloads/BusyBox.html#login)
[_logname_](https://www.busybox.net/downloads/BusyBox.html#logname)
[_logread_](https://www.busybox.net/downloads/BusyBox.html#logread)
[_losetup_](https://www.busybox.net/downloads/BusyBox.html#losetup)
[_lpd_](https://www.busybox.net/downloads/BusyBox.html#lpd)
[_lpq_](https://www.busybox.net/downloads/BusyBox.html#lpq)
[_lpr_](https://www.busybox.net/downloads/BusyBox.html#lpr)
[_ls_](https://www.busybox.net/downloads/BusyBox.html#ls)
[_lsattr_](https://www.busybox.net/downloads/BusyBox.html#lsattr)
[_lsmod_](https://www.busybox.net/downloads/BusyBox.html#lsmod)
[_lzmacat_](https://www.busybox.net/downloads/BusyBox.html#lzmacat)
[_lzop_](https://www.busybox.net/downloads/BusyBox.html#lzop)
[_lzopcat_](https://www.busybox.net/downloads/BusyBox.html#lzopcat)
[_makemime_](https://www.busybox.net/downloads/BusyBox.html#makemime)
[_man_](https://www.busybox.net/downloads/BusyBox.html#man)
[_md5sum_](https://www.busybox.net/downloads/BusyBox.html#md5sum)
[_mdev_](https://www.busybox.net/downloads/BusyBox.html#mdev)
[_mesg_](https://www.busybox.net/downloads/BusyBox.html#mesg)
[_microcom_](https://www.busybox.net/downloads/BusyBox.html#microcom)
[_mkdir_](https://www.busybox.net/downloads/BusyBox.html#mkdir)
[_mkdosfs_](https://www.busybox.net/downloads/BusyBox.html#mkdosfs)
[_mkfifo_](https://www.busybox.net/downloads/BusyBox.html#mkfifo)
[_mkfs.minix_](https://www.busybox.net/downloads/BusyBox.html#mkfs.minix)
[_mkfs.vfat_](https://www.busybox.net/downloads/BusyBox.html#mkfs.vfat)
[_mknod_](https://www.busybox.net/downloads/BusyBox.html#mknod)
[_mkpasswd_](https://www.busybox.net/downloads/BusyBox.html#mkpasswd)
[_mkswap_](https://www.busybox.net/downloads/BusyBox.html#mkswap)
[_mktemp_](https://www.busybox.net/downloads/BusyBox.html#mktemp)
[_modprobe_](https://www.busybox.net/downloads/BusyBox.html#modprobe)
[_more_](https://www.busybox.net/downloads/BusyBox.html#more)
[_mount_](https://www.busybox.net/downloads/BusyBox.html#mount)
[_mountpoint_](https://www.busybox.net/downloads/BusyBox.html#mountpoint)
[_mt_](https://www.busybox.net/downloads/BusyBox.html#mt)
[_mv_](https://www.busybox.net/downloads/BusyBox.html#mv)
[_nameif_](https://www.busybox.net/downloads/BusyBox.html#nameif)
[_nc_](https://www.busybox.net/downloads/BusyBox.html#nc)
[_netstat_](https://www.busybox.net/downloads/BusyBox.html#netstat)
[_nice_](https://www.busybox.net/downloads/BusyBox.html#nice)
[_nmeter_](https://www.busybox.net/downloads/BusyBox.html#nmeter)
[_nohup_](https://www.busybox.net/downloads/BusyBox.html#nohup)
[_nslookup_](https://www.busybox.net/downloads/BusyBox.html#nslookup)
[_od_](https://www.busybox.net/downloads/BusyBox.html#od)
[_openvt_](https://www.busybox.net/downloads/BusyBox.html#openvt)
[_passwd_](https://www.busybox.net/downloads/BusyBox.html#passwd)
[_patch_](https://www.busybox.net/downloads/BusyBox.html#patch)
[_pgrep_](https://www.busybox.net/downloads/BusyBox.html#pgrep)
[_pidof_](https://www.busybox.net/downloads/BusyBox.html#pidof)
[_ping_](https://www.busybox.net/downloads/BusyBox.html#ping)
[_ping6_](https://www.busybox.net/downloads/BusyBox.html#ping6)
[_pipe_progress_](https://www.busybox.net/downloads/BusyBox.html#pipe_progress)
[_pivot_root_](https://www.busybox.net/downloads/BusyBox.html#pivot_root)
[_pkill_](https://www.busybox.net/downloads/BusyBox.html#pkill)
[_popmaildir_](https://www.busybox.net/downloads/BusyBox.html#popmaildir)
[_printenv_](https://www.busybox.net/downloads/BusyBox.html#printenv)
[_printf_](https://www.busybox.net/downloads/BusyBox.html#printf)
[_ps_](https://www.busybox.net/downloads/BusyBox.html#ps)
[_pscan_](https://www.busybox.net/downloads/BusyBox.html#pscan)
[_pwd_](https://www.busybox.net/downloads/BusyBox.html#pwd)
[_raidautorun_](https://www.busybox.net/downloads/BusyBox.html#raidautorun)
[_rdate_](https://www.busybox.net/downloads/BusyBox.html#rdate)
[_rdev_](https://www.busybox.net/downloads/BusyBox.html#rdev)
[_readlink_](https://www.busybox.net/downloads/BusyBox.html#readlink)
[_readprofile_](https://www.busybox.net/downloads/BusyBox.html#readprofile)
[_realpath_](https://www.busybox.net/downloads/BusyBox.html#realpath)
[_reformime_](https://www.busybox.net/downloads/BusyBox.html#reformime)
[_renice_](https://www.busybox.net/downloads/BusyBox.html#renice)
[_reset_](https://www.busybox.net/downloads/BusyBox.html#reset)
[_resize_](https://www.busybox.net/downloads/BusyBox.html#resize)
[_rm_](https://www.busybox.net/downloads/BusyBox.html#rm)
[_rmdir_](https://www.busybox.net/downloads/BusyBox.html#rmdir)
[_rmmod_](https://www.busybox.net/downloads/BusyBox.html#rmmod)
[_route_](https://www.busybox.net/downloads/BusyBox.html#route)
[_rpm_](https://www.busybox.net/downloads/BusyBox.html#rpm)
[_rpm2cpio_](https://www.busybox.net/downloads/BusyBox.html#rpm2cpio)
[_rtcwake_](https://www.busybox.net/downloads/BusyBox.html#rtcwake)
[_run-parts_](https://www.busybox.net/downloads/BusyBox.html#run-parts)
[_runlevel_](https://www.busybox.net/downloads/BusyBox.html#runlevel)
[_runsv_](https://www.busybox.net/downloads/BusyBox.html#runsv)
[_runsvdir_](https://www.busybox.net/downloads/BusyBox.html#runsvdir)
[_rx_](https://www.busybox.net/downloads/BusyBox.html#rx)
[_script_](https://www.busybox.net/downloads/BusyBox.html#script)
[_scriptreplay_](https://www.busybox.net/downloads/BusyBox.html#scriptreplay)
[_sed_](https://www.busybox.net/downloads/BusyBox.html#sed)
[_sendmail_](https://www.busybox.net/downloads/BusyBox.html#sendmail)
[_seq_](https://www.busybox.net/downloads/BusyBox.html#seq)
[_setarch_](https://www.busybox.net/downloads/BusyBox.html#setarch)
[_setconsole_](https://www.busybox.net/downloads/BusyBox.html#setconsole)
[_setfont_](https://www.busybox.net/downloads/BusyBox.html#setfont)
[_setkeycodes_](https://www.busybox.net/downloads/BusyBox.html#setkeycodes)
[_setlogcons_](https://www.busybox.net/downloads/BusyBox.html#setlogcons)
[_setsid_](https://www.busybox.net/downloads/BusyBox.html#setsid)
[_setuidgid_](https://www.busybox.net/downloads/BusyBox.html#setuidgid)
[_sh_](https://www.busybox.net/downloads/BusyBox.html#sh)
[_sha1sum_](https://www.busybox.net/downloads/BusyBox.html#sha1sum)
[_sha256sum_](https://www.busybox.net/downloads/BusyBox.html#sha256sum)
[_sha512sum_](https://www.busybox.net/downloads/BusyBox.html#sha512sum)
[_showkey_](https://www.busybox.net/downloads/BusyBox.html#showkey)
[_slattach_](https://www.busybox.net/downloads/BusyBox.html#slattach)
[_sleep_](https://www.busybox.net/downloads/BusyBox.html#sleep)
[_softlimit_](https://www.busybox.net/downloads/BusyBox.html#softlimit)
[_sort_](https://www.busybox.net/downloads/BusyBox.html#sort)
[_split_](https://www.busybox.net/downloads/BusyBox.html#split)
[_start-stop-daemon_](https://www.busybox.net/downloads/BusyBox.html#start-stop-daemon)
[_stat_](https://www.busybox.net/downloads/BusyBox.html#stat)
[_strings_](https://www.busybox.net/downloads/BusyBox.html#strings)
[_stty_](https://www.busybox.net/downloads/BusyBox.html#stty)
[_su_](https://www.busybox.net/downloads/BusyBox.html#su)
[_sulogin_](https://www.busybox.net/downloads/BusyBox.html#sulogin)
[_sum_](https://www.busybox.net/downloads/BusyBox.html#sum)
[_sv_](https://www.busybox.net/downloads/BusyBox.html#sv)
[_svlogd_](https://www.busybox.net/downloads/BusyBox.html#svlogd)
[_swapoff_](https://www.busybox.net/downloads/BusyBox.html#swapoff)
[_swapon_](https://www.busybox.net/downloads/BusyBox.html#swapon)
[_switch_root_](https://www.busybox.net/downloads/BusyBox.html#switch_root)
[_sync_](https://www.busybox.net/downloads/BusyBox.html#sync)
[_sysctl_](https://www.busybox.net/downloads/BusyBox.html#sysctl)
[_syslogd_](https://www.busybox.net/downloads/BusyBox.html#syslogd)
[_tac_](https://www.busybox.net/downloads/BusyBox.html#tac)
[_tail_](https://www.busybox.net/downloads/BusyBox.html#tail)
[_tar_](https://www.busybox.net/downloads/BusyBox.html#tar)
[_taskset_](https://www.busybox.net/downloads/BusyBox.html#taskset)
[_tcpsvd_](https://www.busybox.net/downloads/BusyBox.html#tcpsvd)
[_tee_](https://www.busybox.net/downloads/BusyBox.html#tee)
[_telnet_](https://www.busybox.net/downloads/BusyBox.html#telnet)
[_telnetd_](https://www.busybox.net/downloads/BusyBox.html#telnetd)
[_test_](https://www.busybox.net/downloads/BusyBox.html#test)
[_tftp_](https://www.busybox.net/downloads/BusyBox.html#tftp)
[_tftpd_](https://www.busybox.net/downloads/BusyBox.html#tftpd)
[_time_](https://www.busybox.net/downloads/BusyBox.html#time)
[_timeout_](https://www.busybox.net/downloads/BusyBox.html#timeout)
[_top_](https://www.busybox.net/downloads/BusyBox.html#top)
[_touch_](https://www.busybox.net/downloads/BusyBox.html#touch)
[_tr_](https://www.busybox.net/downloads/BusyBox.html#tr)
[_traceroute_](https://www.busybox.net/downloads/BusyBox.html#traceroute)
[_true_](https://www.busybox.net/downloads/BusyBox.html#true)
[_tty_](https://www.busybox.net/downloads/BusyBox.html#tty)
[_ttysize_](https://www.busybox.net/downloads/BusyBox.html#ttysize)
[_udhcpc_](https://www.busybox.net/downloads/BusyBox.html#udhcpc)
[_udhcpd_](https://www.busybox.net/downloads/BusyBox.html#udhcpd)
[_udpsvd_](https://www.busybox.net/downloads/BusyBox.html#udpsvd)
[_umount_](https://www.busybox.net/downloads/BusyBox.html#umount)
[_uname_](https://www.busybox.net/downloads/BusyBox.html#uname)
[_uncompress_](https://www.busybox.net/downloads/BusyBox.html#uncompress)
[_unexpand_](https://www.busybox.net/downloads/BusyBox.html#unexpand)
[_uniq_](https://www.busybox.net/downloads/BusyBox.html#uniq)
[_unix2dos_](https://www.busybox.net/downloads/BusyBox.html#unix2dos)
[_unlzma_](https://www.busybox.net/downloads/BusyBox.html#unlzma)
[_unlzop_](https://www.busybox.net/downloads/BusyBox.html#unlzop)
[_unzip_](https://www.busybox.net/downloads/BusyBox.html#unzip)
[_uptime_](https://www.busybox.net/downloads/BusyBox.html#uptime)
[_usleep_](https://www.busybox.net/downloads/BusyBox.html#usleep)
[_uudecode_](https://www.busybox.net/downloads/BusyBox.html#uudecode)
[_uuencode_](https://www.busybox.net/downloads/BusyBox.html#uuencode)
[_vconfig_](https://www.busybox.net/downloads/BusyBox.html#vconfig)
[_vi_](https://www.busybox.net/downloads/BusyBox.html#vi)
[_vlock_](https://www.busybox.net/downloads/BusyBox.html#vlock)
[_volname_](https://www.busybox.net/downloads/BusyBox.html#volname)
[_watch_](https://www.busybox.net/downloads/BusyBox.html#watch)
[_watchdog_](https://www.busybox.net/downloads/BusyBox.html#watchdog)
[_wc_](https://www.busybox.net/downloads/BusyBox.html#wc)
[_wget_](https://www.busybox.net/downloads/BusyBox.html#wget)
[_which_](https://www.busybox.net/downloads/BusyBox.html#which)
[_who_](https://www.busybox.net/downloads/BusyBox.html#who)
[_whoami_](https://www.busybox.net/downloads/BusyBox.html#whoami)
[_xargs_](https://www.busybox.net/downloads/BusyBox.html#xargs)
[_yes_](https://www.busybox.net/downloads/BusyBox.html#yes)
[_zcat_](https://www.busybox.net/downloads/BusyBox.html#zcat)
[_zcip_](https://www.busybox.net/downloads/BusyBox.html#zcip)
...
Скачать сайт, `-r` -- recursive, , `-np` -- no-parent, `-p` -- также скачать всё что требуется для отображения страницы. URL не обязательно должен быть корневой страницей.\
`wget -r -np -p URL`
...
`watch df -h` --- отслеживать заполненность дисков;
...
_Note:_ также у OnlyOffice есть OnlyOffice-Server, предназначенный для организации облачного сервиса, аналога Office365 и SharePoint, с поддержкой распределённой работы, но это уже закрытые платные программы.

> _Отступление про редактирование pdf._\
  Внутренний формат pdf не содержит информации о последовательности символов и слов, а содержит информацию, в каком месте страницы нарисовать тот или символ или слово (и, как следствие, пробелы тоже не хранит). Поэтому все программы для редактирования pdf "додумывают" информацию о том, в какой последовательности идут слова и символы. И если в случае отдельного параграфа можно ожидать (не всегда!), что слова внутри pdf идут в естественном порядке, то уже сами параграфы совсем неоднозначно в каком порядке расположить. С математическими формулами или таблицами ситуация совсем катастрофическая. Поэтому, самый лучший способ редактирования pdf, это сконвертировать pdf в Markdown методом OCR или ИИ, и полученный Markdown сконвертировать в docx.
...
Пример приложения с хранением каждой записи в отдельном файле --- [**Logseq**](https://logseq.com/). Считается свободной альтернативой _Obsidian._ [Гайд по LogSeq](https://vas3k.club/post/25661/).
...
_Note:_ [GraphicsMagick](https://scoop.sh/#/apps?q=graphicsmagick) -- тоже самое, что и ImageMagick, но работает быстрее -- лучше распараллеливается, но, может _~~глючить~~ выполнять обработку с ошибками_. Запускается в точности как ImageMagick, только с префиксом _gm_, а не _magick_, например: `gm mogrify ...`. ~~Использовать стоит только в случае обработки очень большого количества файлов (сотни и больше), и убедившись, что GraphicsMagick выполняет всё идентично с ImageMagick~~ Использовать не стоит, так как для параллелизации есть другие инструменты, например, [GNU parallel](#parallel).

_ImageMagick_ применяется как `magick operation ...`:

* `convert` преобразует изображение и записывает результат в другой указанный файл.

* `mogrify` модифицирует исходный файл.

* `montage` собирает (монтирует) несколько изображений в один файл. По умолчанию уменьшает размер до 120x120. `montage -mode concatenate -tile 1x -geometry 1920x1024 *.jpg out.jpg` -- соединить все изображения в одну вертикальную ленту, отмасштабировав в один размер 1920x1024 с сохранением пропорций.
...
Конвертация всех изображений из папки _images_ в _jpeg_ формат, с масштабированием всех картинок в размер 1024 пикселя по ширине, с сохранением в папку _images-transformed_:
```cmd
magick convert images -resize 1000x JPEG:images-transformed
```
...
Создать видео из набора кадров , _"velmagfield-%03d.png"_ --- шаблон входных файлов:
```cmd
ffmpeg -i "velmagfield-%03d.png" -r 20 -hide_banner -c:a copy -c:v libx264 -crf 18 ^
...
* `-r 20` --- задать частоту кадров видео (соответствует шагу по времени 0.05 секунды);
...
```cmd
ffmpeg -i "velmagfield-%03d.png" -f lavfi -i aevalsrc=0 -hide_banner -r 20 ^
       -max_muxing_queue_size 9999 -c:v libx264 -crf 18 -maxrate 4.5M -preset faster ^
...
Итоговый скрипт для создания видео из набора _png_ картинок, разрешение видео из размера картинок, либо задаётся аргументом скрипта:
...
Установка: в WSL скачивается запускающий _OpenFOAM_ скрипт из официального каталога <http://dl.openfoam.org/docker/> и настраивается; для этого выполнить следующие команды в WSL терминале (открывается командой `wsl` или в таске WSL в WT/Conemu/Cmder):
```sh
...
_Note:_ если изначально OpenFOAM не был закачен с помощью _docker pull ..._, то OpenFOAM будет загружен при первом запуске.
...
В последующем, для запуска _SageMath_ набирать столь сложную команду не потребуется: контейнер можно будет запускать либо из интерфейса _DockerDesktop_, либо из командной строки командой `docker start sage` (см. [Управление Docker](#управление-dockerом)).
...
Сайт <https://lmstudio.ai/> возможно под санкциями, поэтому [webarchive](https://web.archive.org/web/20241112235033/https://lmstudio.ai/).
...
* для увеличения или уменьшения шрифта в интерфейсе использовать _\<Ctrl+\>_ и _\<Ctrl-\>_;
...
Для скачивания моделей применять команды скопированные со страницы <https://ollama.com/library> для выбранной [LLM модели](https://ollama.com/library/qwen2.5-coder:1.5b-instruct-fp16), где команду `run` заменить на `pull`, например:
```cmd
...
Скачивание модели с <https://huggingface.co/> и запуск сервиса с _OpenAI API_ доступного по адресу <http://localhost:8081>
```cmd
vllm serve Qwen/Qwen2.5-Coder-3B-Instruct --port 8081
```

#### Установка и запуск {.unnumbered .unlisted}

```cmd
docker run --name vllm--Qwen--Qwen2.5-Coder-3B-Instruct --runtime nvidia --gpus all -v ~/.cache/huggingface:/root/.cache/huggingface -p 8088:8000 --ipc=host vllm/vllm-openai:latest --model Qwen/Qwen2.5-Coder-3B-Instruct
```
...
Т.е. на каждую модель свой контейнер. Конечно, быстро модельки так не по-перебираешь, но _vLLM_ на такое и не рассчитан, это скорее про постоянно работающий сервис.
...
_Note:_ в предыдущих примерах использовалась разметка `**` для выделения текста, и использовался обратный слэш (`\`) в конце строк, который в Markdown обозначает перенос строки без начала нового параграфа. Также можно использовать бектики `` ` `` и ```` ``` ```` для выделения участков кода. LLM модели понимают разметку Markdown, поэтому такую разметку следует использовать, для усиления акцентов, или форматирования текста с целью лучшей структуризации информации.
...
# Приложение: Консольные терминалы и приложения
...
* Использовать доработанный _Conemu_ --- [Cmder](), где глоббинг встроен в консольные скрипты, которые перед запуском команд осуществляют подмену _wildcards_ на аргументы, как в Linux shell.
...
[Настройка Clink](https://github.com/chrisant996/clink-fzf?tab=readme-ov-file#how-to-install) для нечёткого (fuzzy) [поиска](https://github.com/chrisant996/clink-fzf#how-to-use), автоподстановки `~` и переменных окружения, интеграции с VSCode: скопировать соответствующие скрипты в отдельную постоянную папку и настроить Clink на их использование:
```cmd
...
`%LOCALAPPDATA%\clink\clink_start.cmd` прописать соответствующие `doskey` команды:
```cmd
@echo off
...
Документация <https://neovim.io/doc/user/>; [команды nvim на stackoverflow](https://stackoverflow.com/questions/5400806/what-are-the-most-used-vim-commands-keypresses). Есть множество шпаргалок по командам Vim/Neovim, например: <https://devhints.io/vim>.
...
* выбор стрелками _вверх_/_вниз_ в меню _autocomplete_ при редактировании в режиме INSERT, выбор --- `Ctrl-Y` (_Y_, это от _Yes_);
...
* LSP не только подсвечивает ошибки, но может некоторые из них исправлять --- при этом он высветит сообщение _fix available_. Комбинация `<Leader>ca` (вызывает _:vim.lsp.buf.code_action()_) производит исправление;

* на `<LocalLeader>p` настроен показ истории yank (буфера копи-пасты);

_Note:_ для постоянной смены схемы подсветки её необходимо прописать в конце файла\
...
# Приложение: Загрузка продуктов Microsoft
...
_Note:_ Скачивание с сайта Microsoft Unified Update Platform, это официальный способ получения установочных пакетов и обновлений для продуктов Microsoft.

_Note:_ После создания ISO-образа следует использовать утилиту _Rufus_ для создания загрузочной флешки.
...
[**UUPMediaCreator**](https://github.com/gus33000/UUPMediaCreator), кроссплатформенная утилита, которая позволяет скачать и создать установочный ISO-образ с операционной системой Windows. Этот iso-образ создаётся по официальной методике получения образов и обновлений c официального сайта Microsoft Unified Update Platform.
...
_Note:_ UUPMediaConverter выполняется полчаса.



## ODT --- Office Deployment Tool --- загрузка MS Office

[**ODT**](https://www.microsoft.com/en-us/download/details.aspx?id=49117) --- _Office Deployment Tool_, _Средство Развертывания Office_, используется для выбора конфигурации MSOffice для загрузки, загрузки MSOffice, и установки. [Официальный обзор](https://learn.microsoft.com/ru-ru/microsoft-365-apps/admin-center/overview-office-customization-tool).

### Выбор версии MSOffice

_LTSC (Long-Term Servicing Channel)_ --- это версия Офиса для корпоративного сегмента, где после установки в процессе эксплуатации не должен как-либо меняться функционал или преднастройки, поэтому, это версия Офиса с обновлениями безопасности, но без обновления функционала. Поэтому, при возможности выбора, лучше установить полноценную версию; но, если LTSC-версия уже установлена, то и ладно.

_Office 365_ --- это офис с ежемесячной или ежегодной подпиской, требует постоянного подключения к интернету. Не стоит связываться, если есть такая возможность. _Note:_ Сейчас Microsoft старается продвигать только _Office 365_, а все упоминания об обычном бессрочном автономном Офисе удалены со страниц сайта Microsoft.
...
* Retail --- привычная "коробочная" версия из магазина, сейчас может поставляться в виде электронного ESD ключа. Эту лицензию можно переустановить (не более трёх раз?) на другой компьютер (на старом, естественно, Офис перестанет работать). Должна быть активирована в течении месяца с момента установки. Не может быть активирована сервисом KMS.
* Volume --- для большого количества лицензий (от 5 штук). Использует не ключи, а сервисы лицензий KMS. Нет ограничений на перенос на другой компьютер.
...
**Итого, если не используется MS Access, то Office Home _абсолютно достаточно!_**

_Note:_ Раньше редактор математических формул был только в Professional версии, как обстоит дело сейчас давно никто не проверял.
...
# Приложение: Документация и ссылки
