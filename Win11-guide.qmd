...
# **Этап 1: Установка базового набора рабочих программ** {.unnumbered}
...
Графические программы следует устанавливать либо через _WinGet_, либо через _Chocolatey_ --- примерно одно и тоже. Через winget стоит устанавливать те программы, которым не требуется регулярное обновление, либо которые умеют сами обновляться, например Telegram --- установил и забыл. Если программы требуют регулярного контроля над обновлениями, то это удобнее делать через ChocolateyGUI.

В общем, все не консольные программы ставить через _WinGet_, а через _Chocolatey_ устанавливать то, чего нет в _WinGet_.
...
Несколько слов о разных программах и утилитах, которые не требуют подробного описания установки или использования. Большинство из них были в списках установки выше, т.е. _уже_ были установлены, кроме OpenVPN.
...
* Быстрый переход в конкретные приложения по горячим клавишам: [autohotkey](https://github.com/AutoHotkey/AutoHotkey) (для продвинутых пользователей  --- трудно настроить).
...
Есть плагин [uBlacklist](https://github.com/iorate/ublacklist#ublacklist), который позволяет исключить из выдачи поисковиков (google.com и т.д.) некоторые сайты --- заблокировать их от показа.

## Far Manager и консольный терминал

### Первый уровень сложности настройки
...
* Поменять шрифты можно кликнув правой кнопкой мыши на иконке Far, в свойствах.
* В Far'е можно открывать [ssh-соединения](#sshputty) с поддержкой [X11 приложений](#x-server), для этого в терминальном окне Far (`Ctrl+O`) вызвать две команды:
  ```cmd
  set DISPLAY=127.0.0.1:0.0
  ssh -Y somename@somehost
  ```
...
* В Windows Terminal для создания [ssh-соединения](#sshputty) (с поддержкой [X11 приложений](#x-server)) вызвать следующую команду прямо из _cmd_ (_clink_) сессии:
  ```cmd
  set DISPLAY=127.0.0.1:0.0& ssh -Y somename@somehost
  ```
...
В принципе, на этом можно и остановиться, но для полноценной разработки/программирования рекомендуется установить и настроить нормальный shell [Clink](https://chrisant996.github.io/clink/). В [приложении показана настройка](#консольный-терминал-windowsterminal-clink) Clink в WindowsTerminal.
...
### Вариант для OpenSSH соединения
...
# **Этап 2: Установка и настройка иногда нужных программ** {.unnumbered}
...
_Note:_ [GraphicsMagick](https://scoop.sh/#/apps?q=graphicsmagick) -- тоже самое, что и ImageMagick, но работает быстрее -- лучше распараллеливается, но, может _~~глючить~~ выполнять обработку с ошибками_. Запускается в точности как ImageMagick, только с префиксом _gm_, а не _magick_, например: `gm mogrify ...`. ~~Использовать стоит только в случае обработки очень большого количества файлов (сотни и больше), и убедившись, что GraphicsMagick выполняет всё идентично с ImageMagick~~ Использовать не стоит так как для параллелизации есть другие инструменты, например, [parallel](#parallel).
...
Таблица. Указание [размеров изображения](https://imagemagick.org/script/command-line-processing.php#geometry) для утилит _ImageMagick_, далее показаны на примерах.
...
| **{***size***}{***offset***}** | Specifying the offset (default is _+0+0_). Below, _{size}_ refers to any of the forms above.|
| **{***size***}{***+-***}***x***{***+-***}***y*  | Horizontal and vertical offsets x and y, specified in pixels.<br> Signs are required for both.<br> Offsets are affected by _-gravity_[^gravity] setting.<br> Offsets are not affected by _%_ or other size operators.<br> Note that positive _X_ and _Y_ offsets are in the inward direction<br> towards the center of the image for all _-gravity_ options, except 'center'.<br> For East, _+X_ is left. For South, _+Y_ is up.<br> For SouthEast, _+X_ is left and _+Y_ is up.<br> For center, the normal _X_ and _Y_ directional convention<br> is used (_+X_ is right and _+Y_ is down).|

Подробности для опции _-gravity_ см. в [руководстве по программе](https://imagemagick.org/script/command-line-options.php#gravity).

> Отступление об особенностях экранирования аргументов команд в командной строке в Windows и Linux. В Windows особое значение имеют символы: `%`, `^` (`%` используется для разыменования переменных, а `^` в конце строки указывает на продолжение строки). Они используются следующим способом: там где нужен `%` надо писать `%%`, а `^` должен быть экранирован кавычками, иначе будет проигнорирован. В Linux особые символы `!`, `(`, `)`, `\`; они должны быть экранированы либо кавычками, либо с помощью `\`; и сам символ `\` тоже должен быть экранирован как `\\`. Также, в Windows и Linux одинарные и двойные кавычки имеют противоположную роль: в Linux одинарные кавычки более "сильные", чем двойные; в Windows наоборот. Поэтому при использовании в Windows команд из мира Linux надо заменять `'` на `"`, и наоборот.
...
Вычислить размер рисунка кратный 16 пикселям в большую сторону; эти размеры требуются для генерации видео, т.к. сгенерировать avi/mp4/mkv можно только с размером кадра кратным 16:
```cmd
...
```
Полученный мультик, скорее всего, не будет проигрываться в PowerPoint из-за несовместимых кодеков, в таком случае потребуется сконвертировать в стандартный _mpeg4_:
```cmd
HandBrakeCLI --preset "Production Standard" --encoder mpeg4 -i velmagfield.m4v -o velmagfield.mp4
```

_Note:_ На самом деле _ffmpeg_ умеет сразу делать нормальные видео, надо только подобрать и указать подходящий кодек и опции, см. `ffmpeg --help full`. Для упрощения этой задачи есть несколько графических оболочек для _ffmpeg_: [ffmpeg_batch](https://github.com/eibol/ffmpeg_batch), [clever-FFmpeg-GUI](https://www.videohelp.com/software/clever-FFmpeg-GUI) или [handbrake](https://handbrake.fr/). Это конвертеры видео с _ffmpeg_ по капотом, они могут ровно то, что может _ffmpeg_, а именно: переконвертация видео в другой кодек, другое разрешение, кадрирование, обрезка по времени. Эти приложения могут быть установлены с помощью [_Scoop_](https://scoop.sh/#/apps) или [_Winget_](https://winstall.app/).
...
# **Этап 3: Сложные нужные программы** {.unnumbered}
...
# **Этап 4: Сложные ненужные программы** {.unnumbered}
...
* `{}` --- [строка замены](https://www.gnu.org/software/parallel/parallel.html#options), на место которого будет подставлен аргумент для распараллеливаемой команды;
