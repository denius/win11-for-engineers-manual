...
Версии 2.6.xy не работают со старыми ключами, поэтому, либо скачать предыдущую версию 2.5.xy (2.5.10) с сайта <https://openvpn.net/community-downloads/> и после установки зафиксировать эту версию:
```sh
...
**[OnlyOffice](https://www.onlyoffice.com/ru/desktop.aspx)** -- офисный пакет от Российских разработчиков, участвует в программе импортозамещения. В качестве основного формата использует форматы файлов MSOffice. Умеет редактировать _pdf_. Opensource, кроссплатформенный, имеет версию для Android и iOS. Установка:
```cmd
...
Удобная программа "По одной кнопке". Удобный встроенный поиск моделей, возможность настройки параметров модели при запуске. Умеет загружать pdf/txt прямо при запросе. Но есть два больших минуса: не opensorce, и _лицензия запрещает коммерческое применение!_ А так, для "поиграться", самое то.
...
Для программирования, работы с Markdown, и формул очень хорошо работает модель [qwen2.5-coder](https://ollama.com/library/qwen2.5-coder). Для выполнения задач надо прямо в лоб на русском спрашивать:
...
В этом  приложении нет какого-то особого _RAG_ или _Баз Знаний (Knowledge Base)_ -- прикрепляются файлы "скрепкой", и запускается запрос. Вполне неплохо справляется.
...
Это официальный способ получения обновлений для продуктов Microsoft.

После создания ISO-образа следует использовать утилиту Rufus для создания загрузочной флешки.
