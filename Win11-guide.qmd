...
Retail лицензия --- это привычная "коробочная" версия из магазина, сейчас может поставляться в виде электронного ESD ключа. Эту лицензию можно переустановить (для Офиса не более трёх раз) на другой компьютер (на старом, естественно, лицензия перестанет работать). Должна быть активирована в течении месяца с момента установки. Не может быть активирована сервисом KMS.

Volume (корпоративные) лицензии для большого количества лицензий (от 5 штук) --- поштучно не продаются. Volume лицензии можно активировать с помощью KMS сервисов по сети. Активации с помощью KMS сервиса по локальной сети, это совершенно стандартная процедура, которая происходит автоматически для продуктов с Volume лицензией при правильной настройке свойств локального сетевого подключения (необходимо указать в качестве DNS сервера адрес KMS сервиса). В случае подсоединения через интернет, возможность KMS активации ограничена.
...
_Note:_ Подробное описание по командам можно найти на сайте <https://www.opennet.ru/man_1.shtml>.
...
Команда  | Описание
---------|------------------------------------------
[`[`](https://www.opennet.ru/man.shtml?topic=test), [`[[`](https://www.opennet.ru/man.shtml?topic=test&category=1&russian=5) | проверка типов файлов и сравнение значений
...
[`gunzip`](https://frippery.org/busybox/BusyBox.html#gunzip)               | распаковка файлов
...
[`test`](https://www.busybox.net/downloads/BusyBox.html#test)              | проверка типов файлов и сравнение значений
