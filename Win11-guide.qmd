...
На данный момент все инсталяторы для [продуктов Microsoft загружаются через интернет](https://www.microsoft.com/en-us/download), это официальный рекомендуемый способ установки --- диски отошли в прошлое. [Загрузить Windows](https://www.microsoft.com/en-us/download/windows), [загрузить Office](https://www.microsoft.com/en-us/download/office).

> _Note:_ В данном руководстве приходится расписывать процедуры загрузки продуктов Microsoft, во-первых, из-за санкций, которые ограничивают загрузку напрямую, непосредственно с сайта Microsoft (хотя никакого запрета на продажу или использование лицензий Microsoft не существует), и, во-вторых, для контроля загружаемых версий --- сейчас Microsoft продвигает только _Office 365_, поэтому обычный автономный _Office_ не загрузить напрямую со страниц сайта Microsoft.\
  \
...
Особенности версии _23H2_ _Windows 11_ в [_Приложении загрузка MS Windows_](#uup-unified-update-platform-загрузка-ms-windows).

Подробности выбора версии пакета офиса в [_Приложении Выбор версии MSOffice_](#выбор-версии-msoffice).


## Установка с онлайн активацией лицензии
...
При установке Windows не подключать интернет! --- сказать, что нет интернета. И, соответственно, установка будет идти без учётной записи Microsoft --- будет только локальное имя и пароль. Регион выбрать _US_, и вообще всё _En_, можно только добавить клавиатуру _Ru_, но не делать её первой.
...
Установить _FirefoxESR_: `winget install --id=Mozilla.Firefox.ESR -e`, установить _МастерПароль_, установить [_NoScript_](https://addons.mozilla.org/en-US/firefox/addon/noscript/), [_U-Block Origin_](https://addons.mozilla.org/en-US/firefox/addon/ublock-origin/). Этот браузер будет для работы с конфиденциальными данными, для работы: с почтой, банковскими сайтами, и т.п. Поначалу придётся много раз добавлять сайты в разрешённые в _NoScript_, но их всё же ограниченное число.
...
Для просмотра векторов в окошке _Watch_ нажать `+` и набрать `argv, 4`, где _4_ это длина просматриваемого участка вектора.
...
Для профессиональной разработки на Python, возможно, стоит рассмотреть _проприетарный_ _PyCharm_ --- порог вхождения выше, но и возможностей больше.
```cmd
...
[_\[_](https://frippery.org/busybox/BusyBox.html#test)
[_\[\[_](https://frippery.org/busybox/BusyBox.html#test)
...
[_test_](https://frippery.org/busybox/BusyBox.html#test)
...
[_vi_](https://frippery.org/busybox/BusyBox.html#vi)([tutorial](https://k.japko.eu/busybox-vi-tutorial.html))
...
```cmd
ffmpeg -i "velmagfield-%04d.png" -r 20 -hide_banner -c:a copy -c:v libx264 -crf 18 ^
...
```cmd
ffmpeg -i "velmagfield-%04d.png" -f lavfi -i aevalsrc=0 -hide_banner -r 20 ^
...
_Note:_ Существует [_opensource_](https://github.com/janhq/jan) аналог --- [_Jan_](https://jan.ai/), но, конечно, пока с не столь широким функционалом. Установить можно через [Winget](https://winstall.app/apps/Jan.Jan).
...
[Установка](https://winstall.app/apps/Rufus.Rufus) Rufus:
```cmd
winget install --id=Rufus.Rufus -e
```
...
_Note:_ Раньше (2003) редактор математических формул был только в Professional версии, как обстоит дело сейчас давно никто не проверял. *CHECKME:*
...
# _Приложение:_ Краткое описание команд входящих в BusyBox

Использовано описание с _man_ страниц команд.
...
Команда  | Описание
---------|------------------------------------------
[`[`](https://frippery.org/busybox/BusyBox.html#test), `[[`                | проверка типов файлов и сравнение значений
[`ar`](https://frippery.org/busybox/BusyBox.html#ar)                       | создание, изменение и извлечение из архивов
[`arch`](https://frippery.org/busybox/BusyBox.html#arch)                   | вывод названия аппаратного обеспечения машины (аналогично uname -m)
[`ascii`](https://frippery.org/busybox/BusyBox.html#ascii)                 | набор символов ASCII
[`ash`](https://frippery.org/busybox/BusyBox.html#ash)                     | интерпретатор Unix shell
[`awk`](https://frippery.org/busybox/BusyBox.html#awk)                     | язык для сканирования и обработки шаблонов
[`base32`](https://frippery.org/busybox/BusyBox.html#base32)               | кодирование/декодирование данных в base32 и вывод на стандартный вывод
[`base64`](https://frippery.org/busybox/BusyBox.html#base64)               | кодирование/декодирование данных в base64 и вывод на стандартный вывод
[`basename`](https://frippery.org/busybox/BusyBox.html#basename)           | удаление директории и суффикса из имен файлов
[`bash`](https://frippery.org/busybox/BusyBox.html#bash)                   | GNU Bourne-Again SHell
[`bc`](https://frippery.org/busybox/BusyBox.html#bc)                       | язык для вычислений с произвольной точностью
[`bunzip2`](https://frippery.org/busybox/BusyBox.html#bunzip2)             | компрессор файлов с блочной сортировкой
[`busybox`](https://frippery.org/busybox/BusyBox.html#busybox)             | "Швейцарский нож" для встроенных систем Linux
[`bzcat`](https://frippery.org/busybox/BusyBox.html#bzcat)                 | распаковка файлов на стандартный вывод
[`bzip2`](https://frippery.org/busybox/BusyBox.html#bzip2)                 | компрессор файлов с блочной сортировкой
[`cal`](https://frippery.org/busybox/BusyBox.html#cal)                     | отображение календаря и даты Пасхи
[`cat`](https://frippery.org/busybox/BusyBox.html#cat)                     | объединение файлов и вывод на стандартный вывод
[`cdrop`](https://frippery.org/busybox/BusyBox.html#cdrop)                 | Сброс повышенных привилегий и выполнение команды. Если команда не указана, запускается cmd.exe.
[`chattr`](https://frippery.org/busybox/BusyBox.html#chattr)               | изменение атрибутов файлов в файловой системе Linux
[`chmod`](https://frippery.org/busybox/BusyBox.html#chmod)                 | изменение битов режима файла
[`cksum`](https://frippery.org/busybox/BusyBox.html#cksum)                 | подсчет контрольной суммы и количества байтов в файле
[`clear`](https://frippery.org/busybox/BusyBox.html#clear)                 | очистка экрана терминала
[`cmp`](https://frippery.org/busybox/BusyBox.html#cmp)                     | побайтовое сравнение двух файлов
[`comm`](https://frippery.org/busybox/BusyBox.html#comm)                   | построчное сравнение двух отсортированных файлов
[`cp`](https://frippery.org/busybox/BusyBox.html#cp)                       | копирование файлов и директорий
[`cpio`](https://frippery.org/busybox/BusyBox.html#cpio)                   | копирование файлов в архивы и из них
[`crc32`](https://frippery.org/busybox/BusyBox.html#crc32)                 | вычисление контрольных сумм CRC-32 для указанных файлов
[`cut`](https://frippery.org/busybox/BusyBox.html#cut)                     | удаление секций из каждой строки файлов
[`date`](https://frippery.org/busybox/BusyBox.html#date)                   | вывод или установка системной даты и времени
[`dc`](https://frippery.org/busybox/BusyBox.html#dc)                       | калькулятор с произвольной точностью
[`dd`](https://frippery.org/busybox/BusyBox.html#dd)                       | преобразование и копирование файла
[`df`](https://frippery.org/busybox/BusyBox.html#df)                       | отчет об использовании дискового пространства файловой системы
[`diff`](https://frippery.org/busybox/BusyBox.html#diff)                   | построчное сравнение файлов
[`dirname`](https://frippery.org/busybox/BusyBox.html#dirname)             | удаление последнего компонента из имени файла
[`dos2unix`](https://frippery.org/busybox/BusyBox.html#dos2unix)           | конвертер текстовых файлов из формата DOS/Mac в Unix и наоборот
[`dpkg`](https://frippery.org/busybox/BusyBox.html#dpkg)                   | менеджер пакетов для Debian
[`dpkg-deb`](https://frippery.org/busybox/BusyBox.html#dpkg-deb)           | инструмент для работы с архивами пакетов Debian (.deb)
[`drop`](https://frippery.org/busybox/BusyBox.html#drop)                   | сброс повышенных привилегий и выполнение команды
[`du`](https://frippery.org/busybox/BusyBox.html#du)                       | оценка использования дискового пространства
[`echo`](https://frippery.org/busybox/BusyBox.html#echo)                   | вывод строки текста
[`ed`](https://frippery.org/busybox/BusyBox.html#ed)                       | построчный текстовый редактор
[`egrep`](https://frippery.org/busybox/BusyBox.html#egrep)                 | вывод строк, соответствующих шаблонам
[`env`](https://frippery.org/busybox/BusyBox.html#env)                     | запуск программы в измененной среде
[`expand`](https://frippery.org/busybox/BusyBox.html#expand)               | преобразование табуляции в пробелы
[`expr`](https://frippery.org/busybox/BusyBox.html#expr)                   | вычисление выражений
[`factor`](https://frippery.org/busybox/BusyBox.html#factor)               | факторизация чисел
[`false`](https://frippery.org/busybox/BusyBox.html#false)                 | ничего не делать, с ошибкой
[`fgrep`](https://frippery.org/busybox/BusyBox.html#fgrep)                 | вывод строк, соответствующих шаблонам
[`find`](https://frippery.org/busybox/BusyBox.html#find)                   | поиск файлов в иерархии директорий
[`fold`](https://frippery.org/busybox/BusyBox.html#fold)                   | перенос каждой строки ввода до указанной ширины
[`free`](https://frippery.org/busybox/BusyBox.html#free)                   | отображение объема свободной и используемой памяти в системе
[`fsync`](https://frippery.org/busybox/BusyBox.html#fsync)                 | запись всех буферизованных блоков в файлы на диск
[`getopt`](https://frippery.org/busybox/BusyBox.html#getopt)               | разбор параметров командной строки (улучшенный)
[`grep`](https://frippery.org/busybox/BusyBox.html#grep)                   | вывод строк, соответствующих шаблонам
[`groups`](https://frippery.org/busybox/BusyBox.html#groups)               | вывод групп, в которых состоит пользователь
[`gunzip`](https://frippery.org/busybox/BusyBox.html#gunzip)               | сжатие или распаковка файлов
[`gzip`](https://frippery.org/busybox/BusyBox.html#gzip)                   | сжатие или распаковка файлов
[`hd`](https://frippery.org/busybox/BusyBox.html#hd)                       | дамп в формате ASCII, десятичный, шестнадцатеричный, восьмеричный
[`head`](https://frippery.org/busybox/BusyBox.html#head)                   | вывод первой части файлов
[`hexdump`](https://frippery.org/busybox/BusyBox.html#hexdump)             | дамп в формате ASCII, десятичный, шестнадцатеричный, восьмеричный
[`httpd`](https://frippery.org/busybox/BusyBox.html#httpd)                 | прослушивание входящих HTTP-запросов
[`iconv`](https://frippery.org/busybox/BusyBox.html#iconv)                 | преобразование текста из одной кодировки в другую
[`id`](https://frippery.org/busybox/BusyBox.html#id)                       | вывод реального и эффективного идентификаторов пользователя и группы
[`inotifyd`](https://frippery.org/busybox/BusyBox.html#inotifyd)           | запуск программы при изменениях в файловой системе
[`install`](https://frippery.org/busybox/BusyBox.html#install)             | копирование файлов и установка атрибутов
[`ipcalc`](https://frippery.org/busybox/BusyBox.html#ipcalc)               | вычисление и отображение сетевых настроек по IP-адресу
[`jn`](https://frippery.org/busybox/BusyBox.html#jn)                       | создание соединения директории JUNC с DIR
[`kill`](https://frippery.org/busybox/BusyBox.html#kill)                   | отправка сигнала процессу
[`killall`](https://frippery.org/busybox/BusyBox.html#killall)             | завершение процессов по имени
[`lash`](https://frippery.org/busybox/BusyBox.html#lash)                   | интерпретатор Unix shell
[`less`](https://frippery.org/busybox/BusyBox.html#less)                   | противоположность more
[`link`](https://frippery.org/busybox/BusyBox.html#link)                   | вызов функции link для создания ссылки на файл
[`ln`](https://frippery.org/busybox/BusyBox.html#ln)                       | создание ссылок между файлами
[`logname`](https://frippery.org/busybox/BusyBox.html#logname)             | вывод имени пользователя, вошедшего в систему
[`ls`](https://frippery.org/busybox/BusyBox.html#ls)                       | вывод содержимого директории
[`lsattr`](https://frippery.org/busybox/BusyBox.html#lsattr)               | вывод атрибутов файлов в файловой системе Linux second extended
[`lzcat`](https://frippery.org/busybox/BusyBox.html#lzcat)                 | сжатие или распаковка файлов .xz и .lzma
[`lzma`](https://frippery.org/busybox/BusyBox.html#lzma)                   | сжатие или распаковка файлов .xz и .lzma
[`lzop`](https://frippery.org/busybox/BusyBox.html#lzop)                   | сжатие или распаковка файлов
[`lzopcat`](https://frippery.org/busybox/BusyBox.html#lzopcat)             | распаковка файлов на стандартный вывод
[`make`](https://frippery.org/busybox/BusyBox.html#make)                   | утилита GNU make для управления группами программ
[`man`](https://frippery.org/busybox/BusyBox.html#man)                     | интерфейс к системным справочным руководствам
[`md5sum`](https://frippery.org/busybox/BusyBox.html#md5sum)               | вычисление и проверка MD5 хеш-суммы
[`mkdir`](https://frippery.org/busybox/BusyBox.html#mkdir)                 | создание директорий
[`mktemp`](https://frippery.org/busybox/BusyBox.html#mktemp)               | создание временного файла или директории
[`mv`](https://frippery.org/busybox/BusyBox.html#mv)                       | перемещение (переименование) файлов
[`nc`](https://frippery.org/busybox/BusyBox.html#nc)                       | произвольные TCP и UDP соединения и прослушивание
[`nl`](https://frippery.org/busybox/BusyBox.html#nl)                       | нумерация строк файлов
[`nproc`](https://frippery.org/busybox/BusyBox.html#nproc)                 | вывод количества доступных процессорных единиц
[`od`](https://frippery.org/busybox/BusyBox.html#od)                       | дамп файлов в восьмеричном и других форматах
[`paste`](https://frippery.org/busybox/BusyBox.html#paste)                 | объединение строк файлов
[`patch`](https://frippery.org/busybox/BusyBox.html#patch)                 | применение diff-файла к оригиналу
[`pdpmake`](https://frippery.org/busybox/BusyBox.html#pdpmake)             | управление файлами на основе их зависимостей
[`pdrop`](https://frippery.org/busybox/BusyBox.html#pdrop)                 | сброс повышенных привилегий и выполнение команды. Если команда не указана, запускается PowerShell
[`pgrep`](https://frippery.org/busybox/BusyBox.html#pgrep)                 | поиск или сигнализация процессов по имени и другим атрибутам
[`pidof`](https://frippery.org/busybox/BusyBox.html#pidof)                 | поиск идентификатора процесса запущенной программы
[`pipe_progress`](https://frippery.org/busybox/BusyBox.html#pipe_progress) | отображение точки для индикации активности в канале
[`pkill`](https://frippery.org/busybox/BusyBox.html#pkill)                 | поиск или сигнализация процессов по имени и другим атрибутам
[`printenv`](https://frippery.org/busybox/BusyBox.html#printenv)           | вывод всей или части среды окружения
[`printf`](https://frippery.org/busybox/BusyBox.html#printf)               | форматирование и вывод данных
[`ps`](https://frippery.org/busybox/BusyBox.html#ps)                       | снимок текущих процессов
[`pwd`](https://frippery.org/busybox/BusyBox.html#pwd)                     | вывод имени текущей/рабочей директории
[`readlink`](https://frippery.org/busybox/BusyBox.html#readlink)           | вывод разрешенных символических ссылок или канонических имен файлов
[`realpath`](https://frippery.org/busybox/BusyBox.html#realpath)           | вывод разрешенного пути
[`reset`](https://frippery.org/busybox/BusyBox.html#reset)                 | инициализация терминала
[`rev`](https://frippery.org/busybox/BusyBox.html#rev)                     | реверсирование строк посимвольно
[`rm`](https://frippery.org/busybox/BusyBox.html#rm)                       | удаление файлов или директорий
[`rmdir`](https://frippery.org/busybox/BusyBox.html#rmdir)                 | удаление пустых директорий
[`rpm`](https://frippery.org/busybox/BusyBox.html#rpm)                     | менеджер пакетов RPM
[`rpm2cpio`](https://frippery.org/busybox/BusyBox.html#rpm2cpio)           | извлечение архива cpio из пакета RPM
[`sed`](https://frippery.org/busybox/BusyBox.html#sed)                     | потоковый редактор для фильтрации и преобразования текста
[`seq`](https://frippery.org/busybox/BusyBox.html#seq)                     | вывод последовательности чисел
[`sh`](https://frippery.org/busybox/BusyBox.html#sh)                       | Unix SHell
[`sha1sum`](https://frippery.org/busybox/BusyBox.html#sha1sum)             | вычисление и проверка SHA1 хеш-суммы
[`sha256sum`](https://frippery.org/busybox/BusyBox.html#sha256sum)         | вычисление и проверка SHA256 хеш-суммы
[`sha512sum`](https://frippery.org/busybox/BusyBox.html#sha512sum)         | вычисление и проверка SHA512 хеш-суммы
[`shred`](https://frippery.org/busybox/BusyBox.html#shred)                 | перезапись файла для сокрытия его содержимого и, возможно, удаление
[`shuf`](https://frippery.org/busybox/BusyBox.html#shuf)                   | генерация случайных перестановок
[`sleep`](https://frippery.org/busybox/BusyBox.html#sleep)                 | задержка на указанное количество времени
[`sort`](https://frippery.org/busybox/BusyBox.html#sort)                   | сортировка строк текстовых файлов
[`split`](https://frippery.org/busybox/BusyBox.html#split)                 | разделение файла на части
[`ssl_client`](https://frippery.org/busybox/BusyBox.html#ssl_client)       | SSL-клиент
[`stat`](https://frippery.org/busybox/BusyBox.html#stat)                   | отображение статуса файла или файловой системы
[`strings`](https://frippery.org/busybox/BusyBox.html#strings)             | вывод последовательностей печатных символов в файлах
[`su`](https://frippery.org/busybox/BusyBox.html#su)                       | выполнение команды с подменой пользователя и группы
[`sum`](https://frippery.org/busybox/BusyBox.html#sum)                     | подсчет контрольной суммы и количества блоков в файле
[`sync`](https://frippery.org/busybox/BusyBox.html#sync)                   | синхронизация кэшированных записей с постоянным хранилищем
[`tac`](https://frippery.org/busybox/BusyBox.html#tac)                     | объединение и вывод файлов в обратном порядке
[`tail`](https://frippery.org/busybox/BusyBox.html#tail)                   | вывод последней части файлов
[`tar`](https://frippery.org/busybox/BusyBox.html#tar)                     | утилита для архивирования
[`tee`](https://frippery.org/busybox/BusyBox.html#tee)                     | чтение из стандартного ввода и запись в стандартный вывод и файлы
[`test`](https://frippery.org/busybox/BusyBox.html#test)                   | проверка типов файлов и сравнение значений
[`time`](https://frippery.org/busybox/BusyBox.html#time)                   | выполнение программ и вывод сводки использования системных ресурсов
[`timeout`](https://frippery.org/busybox/BusyBox.html#timeout)             | выполнение команды с ограничением по времени
[`touch`](https://frippery.org/busybox/BusyBox.html#touch)                 | изменение временных меток файла
[`tr`](https://frippery.org/busybox/BusyBox.html#tr)                       | замена или удаление символов
[`true`](https://frippery.org/busybox/BusyBox.html#true)                   | ничего не делать, успешно
[`truncate`](https://frippery.org/busybox/BusyBox.html#truncate)           | уменьшение или увеличение размера файла до указанного размера
[`ts`](https://frippery.org/busybox/BusyBox.html#ts)                       | добавляет временную метку в начало каждой строки ввода
[`tsort`](https://frippery.org/busybox/BusyBox.html#tsort)                 | выполнение топологической сортировки
[`ttysize`](https://frippery.org/busybox/BusyBox.html#ttysize)             | вывод размеров stdin tty, или 80x24
[`uname`](https://frippery.org/busybox/BusyBox.html#uname)                 | вывод информации о системе
[`uncompress`](https://frippery.org/busybox/BusyBox.html#uncompress)       | сжатие или распаковка файлов
[`unexpand`](https://frippery.org/busybox/BusyBox.html#unexpand)           | преобразование пробелов в табуляцию
[`uniq`](https://frippery.org/busybox/BusyBox.html#uniq)                   | вывод или пропуск повторяющихся строк
[`unix2dos`](https://frippery.org/busybox/BusyBox.html#unix2dos)           | конвертер текстовых файлов из формата Unix в DOS/Mac и наоборот
[`unlink`](https://frippery.org/busybox/BusyBox.html#unlink)               | вызов функции unlink для удаления указанного файла
[`unlzma`](https://frippery.org/busybox/BusyBox.html#unlzma)               | сжатие или распаковка файлов .xz и .lzma
[`unlzop`](https://frippery.org/busybox/BusyBox.html#unlzop)               | распаковка файлов
[`unxz`](https://frippery.org/busybox/BusyBox.html#unxz)                   | сжатие или распаковка файлов .xz и .lzma
[`unzip`](https://frippery.org/busybox/BusyBox.html#unzip)                 | просмотр, тестирование и извлечение файлов из архива ZIP
[`uptime`](https://frippery.org/busybox/BusyBox.html#uptime)               | отображение времени работы системы
[`usleep`](https://frippery.org/busybox/BusyBox.html#usleep)               | пауза на N микросекунд
[`uudecode`](https://frippery.org/busybox/BusyBox.html#uudecode)           | декодирование закодированного файла
[`uuencode`](https://frippery.org/busybox/BusyBox.html#uuencode)           | кодирование файла в текст, пригодный для электронной почты
[`vi`](https://frippery.org/busybox/BusyBox.html#vi)                       | Vi, текстовый редактор для программистов
[`watch`](https://frippery.org/busybox/BusyBox.html#watch)                 | периодическое выполнение программы с отображением вывода на весь экран
[`wc`](https://frippery.org/busybox/BusyBox.html#wc)                       | подсчет количества строк, слов и байтов для каждого файла
[`wget`](https://frippery.org/busybox/BusyBox.html#wget)                   | неинтерактивный загрузчик из сети
[`which`](https://frippery.org/busybox/BusyBox.html#which)                 | поиск команды
[`whoami`](https://frippery.org/busybox/BusyBox.html#whoami)               | вывод эффективного идентификатора пользователя
[`whois`](https://frippery.org/busybox/BusyBox.html#whois)                 | запрос информации WHOIS
[`xargs`](https://frippery.org/busybox/BusyBox.html#xargs)                 | построение и выполнение командных строк из стандартного ввода
[`xxd`](https://frippery.org/busybox/BusyBox.html#xxd)                     | создание hexdump или обратное преобразование
[`xz`](https://frippery.org/busybox/BusyBox.html#xz)                       | сжатие или распаковка файлов .xz и .lzma
[`xzcat`](https://frippery.org/busybox/BusyBox.html#xzcat)                 | сжатие или распаковка файлов .xz и .lzma
[`yes`](https://frippery.org/busybox/BusyBox.html#yes)                     | повторяющийся вывод строки до завершения^[output a string repeatedly until killed]
[`zcat`](https://frippery.org/busybox/BusyBox.html#zcat)                   | сжатие или распаковка файлов^[compress or expand files]
...
|cdrop             | Drop elevated privileges and run a command. If no command is provided run cmd.exe. |
...
|drop              | drop elevated privileges and run a command |
...
|fsync             | Write all buffered blocks in files to disk |
...
|inotifyd          | run some program on filesystem changes |
|install           | copy files and set attributes |
|ipcalc            | Calculate and display network settings from IP address |
|jn                | Create directory junction JUNC to DIR |
|kill              | send a signal to a process |
|killall           | kill processes by name |
|lash              | unix shell interpreter |
...
|lzopcat           | expand files to stdout |
...
|pdpmake           | maintain files based on their dependencies |
|pdrop             | drop elevated privileges and run a command. If no command is provided run PowerShell |
...
|ttysize           | print dimensions of stdin tty, or 80x24 |
