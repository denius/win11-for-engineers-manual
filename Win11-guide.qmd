...
* _UBlock_ _Origin._  _"Must have"_ для всех браузеров, без вариантов! [_Edge_](https://microsoftedge.microsoft.com/addons/detail/ublock-origin/odfafepnkmbhccpbejgmiehpchacaeak), [_Chrome_/_Chromium_](https://chromewebstore.google.com/detail/ublock-origin/cjpalhdlnbpafiamejdnhcphjbkeiagm), [_Firefox_](https://addons.mozilla.org/en-US/firefox/addon/ublock-origin/). После установки плагина в настройках дополнительно включить фильтр _Ru_ в табе _Filter lists_ в самом низу в _Regions, languages_.
...
**Markdown** поддерживается в VSCode "из коробки", в том числе, есть отображение картинок, формул, сносок, и проч. <https://code.visualstudio.com/docs/languages/markdown>. Встроенный в VSCode Markdown Preview можно настраивать расширениями.

Открыть превью: `Ctrl+Shift+V`.
...
* имеет [стили](https://quarto.org/docs/output-formats/pdf-basics.html#document-class), для оформления [статей](https://quarto.org/docs/output-formats/ms-word.html), [презентаций](https://quarto.org/docs/presentations/powerpoint.html), [сайтов](https://quarto.org/docs/websites/), и др.;
...
```
После отредактировать в конфиге `%UserProfile%\AppData\Roaming\tlrc\config.toml` в секции `[cache]` опцию `languages = ['ru','en']`, чтобы, по возможности, были справки на русском. _Note:_ при первом запуске скачивает с инета архив со справками, и распаковывает, меньше минуты.
...
```cmd
fd --glob "task*.c" Projects
fd -g "task*.c" Projects
```
...
Поиск и по системным (скрытым) файлам тоже:
```cmd
...
_Note:_ `fd` считает свой шаблон _ignorecase_, только если в шаблоне нет заглавных букв; иначе шаблон _casesensitive_. Опция `-i` делает _ignorecase_. Аналогично для `rg`.

_Note:_ `fd` в поиске учитывает спецификации настроек проектов, _git_, и прочего; то если в `.gitignore` указаны файли или папки, то `fd` пропустит их, даже если они удовлетворяют критериям поиска. Для обхода есть опция `-I`, `--no-ignore`. Аналогично для `rg`.
...
`rg` --- поиск по текстовым файлам. `rga` ([rg all](https://github.com/phiresky/ripgrep-all)) --- поиск по: текстовым файлам, pdf, docx (но не doc).
...
Вывести контекст найденного, это опции `A`/`B`/`C` -- _After_, _Before_, _Context_. Следующее выражение ищет команды над _init.lua_ в полной истории команд и выводит не только саму команду, но и ближайшие 5. Эта команда, так же, показывает особенность регулярных выражений -- в них точка обозначает любой символ, а вот для поиска самой точки её необходимо экранировать:
```cmd
...
_Note:_ в Neovim за _синтаксис_ отвечает плагин [_treesitter_](https://tree-sitter.github.io/tree-sitter/), это парсер; в установленной конфигурации kickstart.nvim он уже установлен; treesitter самостоятельно заботится парсинге всевозможных форматов. За _семантику_ отвечает встроенный плагин [_lsp.nvim_](https://neovim.io/doc/user/lsp.html); _LSP_ --- это аббревиатура для [_Language-Server-Protocol_](https://microsoft.github.io/language-server-protocol/overviews/lsp/overview/), универсального сервиса, который обеспечивает: проверку корректности кода и поиск ошибок, автодополнение, переход к определению, подсказки и т.д.; LSP --- это, де факто, стандарт в отрасли разработки, применяется во всех современных IDE и редакторах, в VSCode в том числе. Сервисы LSP для необходимых форматов устанавливаются отдельно, см. далее.
...
После выйти и заново войти в nvim, и набрать `:Mason` для запуска менеджера _LSP_-сервисов для настройки поддерживаемых языков программирования и форматов. В нём найти (`/`) строчку `clangd` и нажать `i` для установки поддержки языка C/C++; аналогично установить `fortls` для Fortran, `python-lsp-server` и `pyright` для Python. Для установки других языков см. список существующих [LSP серверов](https://microsoft.github.io/language-server-protocol/implementors/servers/].
...
LTSC () --- это версия Офиса для корпоративного сегмента, где после установки в процессе не должен как-либо меняться функционал или даже настройки, поэтому эта версия Офиса с обновлениями безопасности, но без обновления функционала. Поэтому, при возможности выбора, лучше установить полноценную версию; но, если LTSC-версия уже установлена, то и ладно.
