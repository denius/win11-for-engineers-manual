...
format:
  pdf:
    ...
    mainfont: OpenSansSemiCondensed Nerd Font
    sansfont: OpenSansSemiCondensed Nerd Font
    monofont: UbuntuCondensed Nerd Font # All Full!
    ...
    monofontoptions: 
      ...
      # - FakeStretch=.7
      # - FakeStretch=.75
      # - CharacterWidth=Half # Not worked
...
Установить **[Far](https://www.farmanager.com/)** и **[WindowsTerminal](https://learn.microsoft.com/ru-ru/windows/terminal/install)**:
```sh
scoop install main/7zip
winget install --id=FarManager.FarManager -e
winget install --id=Microsoft.WindowsTerminal -e
...
scoop install extras/copyq
```
Для автозапуска CopyQ при загрузке Windows, необходимо скопировать ссылку на CopyQ в папку автозапуска, для этого в меню Start правой кнопкой по нему выбрать _Open file location_, скопировать ярлык, и _Paste_ его в папку автозапуска, открываемую по _shell:startup_ в `<Win+R>`.
...
Помимо привычного RDP --- Remote Desktop Protocol, существуют несколько альтернативных инструментов для удалённого подсоединения.

Один из них, это кросплатформенный **[rustdesk](https://rustdesk.com/)**, позволяющий посоединяться к компьютеру локальной сети. Также, через сервер, возможен коннект к компьютеру подсоединённому к интернету. Установка клиента:
```cmd
...
* ~~Чем свежее версия офиса --- тем лучше~~ Пора обновить свой офисный пакет, ему уже больше 10 лет, там всё стало намного удобнее.
...
[Использование](https://pandoc.org/demos.html):
```cmd
...
Cmder самостоятельно создаёт переменную _HOME_. В _Conemu_ переменную _HOME_ можно определить в настройках _Startup > Environment_ как `HOME=%HOMEDRIVE%%HOMEPATH%` или `HOME=%UserProfile%`.
