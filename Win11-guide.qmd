...
> В общем, все консольные программы устанавливать через Scoop, все не консольные программы ставить через _WinGet_, а через _Chocolatey_ устанавливать то, чего нет в _WinGet_.

_Note:_ *Видимость установленных программ менеджерами программ.* Scoop "видит" только те программы, что установил сам. Winget помимо программ, которые были им установлены, также "видит" программы, установленные через Chocolatey; для Chocolatey обратное неверно --- если программа установлена не через Chocolatey, то она её не "видит".
...
_Настройка_: проще всего скопировать ключи прямо вместе с их папкой, в `C:\Program Files\OpenVPN\config`; OpenVPN увидит их (после перезапуска?) и будет показывать в виде отдельной папки конфигурации (появится возможность выбирать из нескольких ключей).\
(_Note_: И ни в коем случае не пытаться загрузить ключи через меню интерфейса _Import > Import File..._, потому что в этом случае будет скопирован только один файл из трёх!)
...
Так же в приложении показано [использование оболочки **Clink**](#использование-clink).
...
Установка: `scoop install main/time`.
