...
# **Этап 1: Установка базового набора рабочих программ** {.unnumbered .unlisted}
...
_Disclaimer:_ в последние годы стало возможным приобрести недорогие OEM лицензии на продукты Microsoft, на Озоне или Вайлбериз, поэтому у пользователей появился выбор между вознёй со скачиванием и применением KMS утилит, и спокойной установкой лицензионного софта. Из минусов установки лицензионных продуктов --- потребуется создание учётной записи на сайте Microsoft для онлайн регистрации продуктов --- регистрация оффлайн стала практически невозможна. Ну, и, лицензии необходимо покупать только у проверенных продавцов со множеством отзывов, но это обычные издержки электронной торговли.

На данный момент все инсталяторы для продуктов Microsoft скачиваются с интернета, это официальный рекомендуемый способ установки --- диски отошли в прошлое.
...
* Поведение `<Alt+Tab>` --- показывать ли отдельные Табы в списке окон по `<Alt+Tab>`: _Settings > System > Mutitasking: Show tabs from apps..._, установить в _Don't show tabs_, тогда по `<Alt+Tab>` будут показываться только окна, без составляющих их табов.
...
Для очень продвинутых пользователей может быть интересна утилита [AutoHotKey](https://www.autohotkey.com/), которая позволяет запрограммировать любые действия на любые клавиши.


# Установка программ

Менеджеры программ для Windows: [**Scoop**](https://scoop.sh/#/) (консольная), [**WinGet**](https://github.com/microsoft/winget-cli) (консольная), [**Chocolatey**](https://chocolatey.org/) (есть GUI). Потребуются все три менеджера, так как не все программы есть в каком-то одном. На сайте <https://repology.org/projects/> есть сведения практически обо всех существующих программах распространяемых через интернет. Также там можно определить в каких репозиториях есть интересующая программа.
...
**_Note:_ Далее в этом руководстве будут рассматриваться _только программы с открытым исходным кодом_, за исключением следующих _проприетарных_ программ: MacriumReflect, PyCharm, LMStudio.**
...
Несколько слов о разных программах и утилитах, которые не требуют подробного описания установки или использования. Большинство из них были в списках установки выше, т.е. уже были установлены, кроме OpenVPN.


### OpenVPN

[**OpenVPN**](https://en.wikipedia.org/wiki/OpenVPN)\ версии 2.6.xy не работает со старыми ключами, т.к. в ней изменились некоторые параметры, и пароль стал обязательным для ключей. Поэтому, либо скачать предыдущую версию 2.5.xy (2.5.10) с сайта <https://openvpn.net/community-downloads/> и после установки зафиксировать эту версию:
```sh
winget pin add OpenVPNTechnologies.OpenVPN
```
Либо отредактировать _.ovpn_ файл: убрать строчку _compress_. FIXME:, CHECKME:!!!
...
Офисные программы рассмотрены в отдельной главе. Написание программных кодов предполагается в VSCode, настройка которого рассмотрена в отдельной главе.
...
Установить _FirefoxESR_: `choco install -y firefoxesr`, установить _МастерПароль_, установить [_NoScript_](https://addons.mozilla.org/en-US/firefox/addon/noscript/), [_U-Block Origin_](https://addons.mozilla.org/en-US/firefox/addon/ublock-origin/). Этот браузер будет для работы с конфиденциальными данными, для работы: с почтой, банковскими сайтами, и т.п.
...
Установить [**Far**](https://www.farmanager.com/) и [**WindowsTerminal**](https://learn.microsoft.com/ru-ru/windows/terminal/install); и запускать Far в WindowsTerminal, в котором можно настроить цвета/палитры, шрифты и т.п. Установка:
```sh
scoop install main/7zip
winget install --id=FarManager.FarManager -e
winget install --id=Microsoft.WindowsTerminal -e
...
* [Использовать _PuTTY_ в Windows Terminal](https://github.com/microsoft/terminal/issues/1917#issuecomment-737710268): для того, чтобы _PuTTY_ открывал соединение и показывал всё в Windows Terminal, необходимо настроить новый профиль, в котором будет вызываться команда `plink` (_C:\\Program Files\\PuTTY\\plink.exe_), эта команда, собственно, и осуществляет непосредственное соединение для _PuTTY._

* Дополнительные цветовые схемы см. в <https://github.com/mbadolato/iTerm2-Color-Schemes>. [Установка](https://learn.microsoft.com/ru-ru/windows/terminal/customize-settings/color-schemes#creating-your-own-color-scheme) дополнительных схем осуществляется вставкой соответствующего `json`-фрагмента настроек цветовой схемы в секцию `"schemes":` в общей конфигурации WindowsTerminal `settings.json`, которая открывается для редактирования по нажатию на "Шестерёнку" в левом нижнем углу в меню _Settings_.\
  Просмотреть цвета текущей схемы можно командой: `colortool -c`,  которая устанавливается: `scoop install main/colortool`.

* Для того, чтобы _Far_ показывал другие цветовые схемы кроме синей, необходимо отжать галочку в меню _Far_: _F9 > Options > Colors: Palette > Classic Palette: Off_.
...
* [Настройка](#консольный-терминал-windowsterminal-clink) Clink в WindowsTerminal.
...
#### Отступление про использование _WSL_: {.unnumbered .unlisted}
...
Установить оба, и [OpenSSH](https://www.openssh.com/), и [Putty](https://www.chiark.greenend.org.uk/~sgtatham/putty/):
```sh
scoop install main/openssh
winget install --id=PuTTY.PuTTY -e
```
...
2. Скопировать ключ на целевой Linux:
   ```sh
...
_Note:_ Запускать VcXsrv следует командой _XLaunch_ из меню _Пуск_; при запуске _XLaunch (VcXsrv)_ ставить галочку "Disable Access Control", это требуется для разрешения на запуск графических приложений с _любого_ Linux узла.
...
Один из них, это _кроссплатформенный_ [**RustDesk**](https://rustdesk.com/), позволяющий подсоединяться к компьютеру локальной сети. Также, через сервер, возможен коннект к _любому_ компьютеру подсоединённому к интернету. Установка клиента:
```cmd
...
# **Этап 2: Установка и настройка иногда нужных программ** {.unnumbered .unlisted}
...
Создайте файл tasks.json для настройки компиляции: открыть командную палитру `<Ctrl+Shift+P>`, ввести _Tasks: Configure Task_, выбрать _Create tasks.json file from template_, выбрать _Others_, отредактировать на:
```json
{
...
_Note:_ [GraphicsMagick](https://scoop.sh/#/apps?q=graphicsmagick) -- тоже самое, что и ImageMagick, но работает быстрее -- лучше распараллеливается, но, может _~~глючить~~ выполнять конвертацию с ошибками_. Запускается в точности как ImageMagick, только с префиксом _gm_, например: `gm mogrify ...`. Использовать стоит только в случае обработки очень большого набора файлов (сотни и больше), и убедившись, что GraphicsMagick выполняет всё идентично с ImageMagick.

_ImageMagick_ применяется как `magick operation ...`, но можно префикс `magick` опускать и применять непосредственно соответствующие утилиты:
...
_Note:_ Порядок опций имеет значение --- несколько действий будут выполняться друг за другом, опции типа _-gravity_ или указания цвета имеют влияние только на следующие за ними команды.
...
| *width***x**                  | Width given, height automagically selected to preserve aspect ratio.                                    |
...
| *width***x***height***\>**    | Shrinks an image with dimension(s) **larger** than\                                                     |
|                               | the corresponding width and/or height argument(s).                                                      |
| *width***x***height***\<**    | Enlarges an image with dimension(s) **smaller** than<br>the corresponding width and/or height argument(s). |
...
| **{***size***}{***+-***}***x***{***+-***}***y*  | Horizontal and vertical offsets x and y, specified in pixels.<br>Signs are required for both.<br>Offsets are affected by -gravity[^gravity] setting.<br>Offsets are not affected by % or other size operators.<br>Note that positive X and Y offsets are in the inward direction<br>towards the center of the image for all -gravity[^gravity] options, except 'center'.<br>For East, +X is left. For South, +Y is up.<br>For SouthEast, +X is left and +Y is up.<br>For center, the normal X and Y directional convention<br>is used (+X is right and +Y is down). |

[^gravity]: https://imagemagick.org/script/command-line-options.php#gravity

#### Примеры

Конвертация всех _jpg_ в меньшее качество -- _75_ (по умолчанию 92), задать имена выходных файлов как _*-converted.jpeg_:
```cmd
convert -quality 75 -set filename: "%t" "%[filename:fname]-converted.jpeg" *.jpg *.jpeg
```
...
Конвертация всех изображений из папки _images_ в _jpeg_ формат, с масштабированием всех картинок в размер 1024 пикселя по ширине:
```cmd
convert images -resize 1000x JPEG:images-transformed
```
...
```cmd
mogrify -gravity Center -crop 1920x1072+40+100 +repage *.png
```

Отмасштабировать в размер 1024x768 с сохранением пропорций, недостающие поля нарастить белым цветом от центра (для прозрачности использовать _none_), указать тип [_png_ палитры цветов](https://www.w3.org/TR/png/#table111) Truecolor без прозрачности (для палитры с прозрачностью задать _6_):
```cmd
mogrify -resize 1920x1072 -background white -gravity Center -extent 1920x1072  +repage -define 'png:color-type=2' *.png
```

Вычислить размер рисунка кратный 16 пикселям в большую сторону; эти размеры требуются для генерации видео, т.к. сгенерировать avi/mp4/mkv можно только размером кадра кратным 16:
```cmd
convert 0000.png  -format '%[fx:16*int((w+15)/16)]x%[fx:16*int((h+15)/16)]' info:
```

Создать видео из набора кадров (их размер должен быть кратным 16), _20_ -- это частота кадров видео (соответствует шагу по времени 0.05 секунды), _"velmagfield-%03d.png"_ шаблон входных файлов:
```cmd
ffmpeg -r 20 -i "velmagfield-%03d.png" -c:v libx264 -preset veryslow -vf scale=-1:1072 velmagfield.m4v
```
Полученный мультик, скорее всего, не будет проигрываться в PowerPoint из-за несовместимых кодеков, в таком случае сконвертировать в стандартный _mpeg4_:
```cmd
HandBrakeCLI --preset "Production Standard" --encoder mpeg4 -i velmagfield.m4v -o velmagfield.mp4
```

_Note:_ На самом деле _ffmpeg_ умеет сразу делать нормальные видео, надо только подобрать и указать подходящий кодек и опции, как это делают графические оболочки для _ffmpeg_: [ffmpeg_batch](https://github.com/eibol/ffmpeg_batch), [clever-FFmpeg-GUI](https://www.videohelp.com/software/clever-FFmpeg-GUI) или [handbrake](https://handbrake.fr/). Это конвертеры видео с _ffmpeg_ по капотом, они могут ровно то, что может _ffmpeg_, а именно: переконвертация видео в другой кодек, другое разрешение, кадрирование, обрезка по времени. Эти приложения могут быть установлены с помощью [_Scoop_](https://scoop.sh/#/apps) или [_Winget_](https://winstall.app/).

_Note:_ Устаревший вариант предыдущей команды:
```cmd
...
Итоговый скрипт для создания видео из набора _png_ картинок:
```sh
#!/bin/sh
...
gm mogrify -trim -resize 100% -bordercolor white -border 8x8 +repage *.png

geom=$(convert "0000.png"  -format '%[fx:16*int((w+15)/16)]x%[fx:16*int((h+15)/16)]' info:)

# mogrify -resize $geom^ -gravity Center -fill white +repage -define 'png:color-type=2' *.png
mogrify -resize $geom^ -background white -gravity Center -extent $geom  +repage -define 'png:color-type=2' *.png
# mogrify -gravity Center -extent $geom  +repage -define 'png:color-type=2' *.png

# mogrify -crop 1408x1408+4+4 +repage -define "png:color-type=2" *.png
# mogrify -crop 608x608+170+20 +repage -define "png:color-type=2" *.png
#mogrify -crop 1488x1040+0+0 +repage -define "png:color-type=2" *.png

# png2yuv -f 10 -Ip -j '%04d.png' | yuv2lav -b 2048 -o "$PWD1/export.avi"
png2yuv -f 20 -Ip -j '%04d.png' | yuv2lav -b 2048 -o "$PWD1/export.avi"

#png2yuv -f 10 -Ip -j '%09d.png' | yuv2lav -q 70 -b 2048 -o "$PWD1/export.avi"

#ffmpeg -r 10 -i "tpaHTIUw3000000%03d.png" -c:v libx264 -preset veryslow -vf scale=-1:1072 "$PWD1/export.m4v"
...
_Note:_ Собрать все картинки в текущей директории в анимированный _gif_. Опция _-delay_ задаёт частоту кадров в форматах: _N_ по 10 миллисекунд, или, _KxL_: _L_ кадров на _K_ секунд (для дробных частот). По умолчанию _-delay 1_ --- 100 кадров в секунду, но эта величина зависит от версии ImageMagick, поэтому лучше указывать её явно.
```cmd
magick *.jpg movie.gif
...
[**Gimp**](https://docs.gimp.org/2.4/ru/gimp-concepts-usage.html) -- open source замена фотошопу.
...
# **Этап 3: Сложные нужные программы** {.unnumbered .unlisted}
...
Установка: в WSL скачивается запускающий _OpenFOAM_ скрипт из официального каталога <http://dl.openfoam.org/docker/> и настраивается; для этого выполнить следующие команды в WSL терминале (открывается командой `wsl` или в таске WSL в Cmder/Conemu):
```sh
...
https://alternativeto.net/software/cloudcompare/about/

https://www.meshlab.net/ -- MeshLabthe open source system for processing and editing 3D triangular meshes.\
...
калькулятор с широчайшими возможностями: единицы измерения, интервальная арифметика, полиномы, решение уравнений, простое дифференцирование и интегрирование, матрицы и вектора! [Примеры выражений](https://github.com/Qalculate/libqalculate/#examples-expressions).\
...
# **Этап 4: Сложные ненужные программы** {.unnumbered .unlisted}
...
Существует небольшое число инструментов, непосредственно выполняющих LLM: [llama.cpp](https://github.com/ggerganov/llama.cpp), [Ollama](https://github.com/ollama/ollama), плюс какие-то графические --- это LLM провайдеры. Но абсолютное большинство [программ](https://github.com/vince-lam/awesome-local-llms#open-source-local-llm-projects) для запуска LLM --- это  интерфейсы, графические "оболочки", которые запускают тот или иной LLM провайдер.

При передаче модели запроса используются [_шаблоны_](https://huggingface.co/docs/transformers/main/en/chat_templating), которые преобразуют запрос в специфически отформатированный текст, лучше распознаваемый моделями. Некоторые программы используют свои шаблоны (LM Studio), некоторые используют шаблоны, предоставляемые авторами модели, или предоставляемые библиотеками (Ollama). От шаблона очень сильно зависит качество ответа модели, поэтому, при использовании одной и той же модели в разных приложениях результаты могут сильно разниться.

Далее рассмотрены несколько таких программ, из тех что попроще.
...
Удобная программа "По одной кнопке". Удобный встроенный поиск моделей, возможность настройки параметров модели при запуске. Умеет загружать pdf/txt прямо при запросе. Но есть два больших минуса: не opensorce -- _проприетарная_, и _лицензия запрещает коммерческое применение!_ А так да, для "поиграться", самое то.
...
Поэтому, в данном руководстве будут рассмотрены оба варианта настройки терминала: и _Cmder_, и _Conemu+BusyBox_. Если Conemu уже установлен и настроен, то предлагается доустановить и настроить BusyBox и Clink, с тем, чтобы получить удобный консольный терминал. Если же Conemu/Cmder не установлен, то использовать Cmder, как более активно развивающийся и прогрессивный.
...
[**Clink**](https://mridgers.github.io/clink/) --- это дополнение для _Cmd.exe_, интегрирующее следующие [возможности](https://chrisant996.github.io/clink/clink.html#features): автодополнение, подсветка синтаксиса, история команд и поиск по ней, поиск файлов и папок, история переходов по папкам, алиасы. (_Note:_ _Clink_ не работает внутри _Far_, т.е в терминале _Far_ по `<Ctrl+O>` все супер-пупер автодополнения и поиск от Clink не сработают --- для работы _Clink_ должна использоваться отдельная консоль.)
...
```
(это одна строка, только она не помещается на ширину формата, поэтому записана в две строки).
...
_Note:_ suppress Clink startup message: `clink set clink.logo none`

## Консольный терминал WindowsTerminal + Clink

Для настройки Clink в WindowsTerminal для использования без Cmder/Conemu надо воспользоваться рецептом установки [Conemu+BusyBox+Clink](#установка-консольного-терминала-conemubusyboxclink), но без Conemu:
```cmd
scoop install main/busybox
scoop install main/clink
scoop install main/clink-completions
...
Все вышеизложенные [настройки относительно Clink](#донастройка-clink) актуальны, останется только настроить алиасы и профиль в WindowsTerminal.
...
* Алиасы в Windows создаются командой [`doskey`](https://learn.microsoft.com/ru-ru/windows-server/administration/windows-commands/doskey). В стартап-файле для Clink\
  `%LOCALAPPDATA%\clink\clink_start.cmd` прописать `doskey` команды:
  ```cmd
  @echo off
...
* Профиль для Clink в WindowsTerminal проще всего получить сдублировав профиль для _cmd.exe_, после чего в новом профиле в качестве запускаемой команды прописать:
  ```cmd
  cmd.exe /k "clink inject"
  ```

## Замечание про _HOME_ {.unnumbered .unlisted}
...
Утилита [`fd`](https://github.com/sharkdp/fd#how-to-use) в качестве шаблона поиска, по умолчанию, использует [регулярные выражения](https://docs.rs/regex/latest/regex/#syntax), поэтому вместо классического шаблона `find -iname '*PATTERN*'` следует использовать ~~банальный~~ удобный:
```cmd
fd PATTERN
```
то есть для поиска курсовой набрать: `fd курсов`, или, если не нашлось, может так поможет: `fd kursov`.

`fd` считает свой шаблон _ignorecase_, если только в шаблоне нет заглавных букв, иначе шаблон _casesensitive_. Опция `-i` обозначает _ignorecase_.

Для поиска по привычным шаблонам со звёздочкой (`*`) применяется опция `-g`/`--glob`:
```cmd
fd --glob "task*.py" Projects
...
_rg all_ --- поиск по документам: pdf, docx (но не doc), html, ipynb, внутри архивов.
...
или, для поиска в _docx_, набирать:
```cmd
rga отчет
```

Опять же, аналогично `fd`, поиск по скрытым файлам с опцией `-h`/`--hidden`.
...
Документация <https://neovim.io/doc/user/>; [команды nvim на stackoverflow](https://stackoverflow.com/questions/5400806/what-are-the-most-used-vim-commands-keypresses). Есть превосходная шпаргалка по командам Vim/Neovim <https://devhints.io/vim>.
...
# Приложение: UUP --- Unified Update Platform

_Note:_ Это официальный способ получения обновлений для продуктов Microsoft.

_Note:_ После создания ISO-образа следует использовать утилиту Rufus для создания загрузочной флешки.

* [**UUPMediaCreator**](https://github.com/gus33000/UUPMediaCreator) позволяет ~~скачать~~ создать установочный ISO-образ с операционной системой Windows. Это образ созданный по официальной методике получения образов и обновления c Unified Update Platform, скачанных с официального сайта Microsoft. [Установка](https://winstall.app/apps/ITDevTeam.UUPMediaCreator):
  ```
  winget install --id=ITDevTeam.UUPMediaCreator -e
  ```
  После установки запустить _UUPMediaCreator_, выбрать образ, после чего компоненты скачаются и будет создан ISO-образ.

* **<https://uupdump.net/>** сайт для создания скрипта, который создаст загрузочный образ Windows. Сайт, возможно, недоступен в России, в этом случае следует воспользоваться другим вариантом из этого раздела.\
  Использование: на сайте выбрать желаемую версию, понажимать далее, в процессе отказаться от скачивания последних обновлений, и, скачать архив. Этот архив распаковать в папку на диске с достаточным количеством свободного места (10&nbsp;Гб?), и запустить скрипт из распакованного архива, `uup_download_windows.cmd` от имени Администратора. [Руководство](https://www.comss.ru/page.php?id=9320).


# Приложение: [ODT](https://www.microsoft.com/en-us/download/details.aspx?id=49117) --- Office Download Tool

Изменить регион для ODT на US:
```cmd
reg add "HKCU\Software\Microsoft\Office\16.0\Common\ExperimentConfigs\Ecs" /v "CountryCode" /t REG_SZ /d "std::wstring|US" /f
```
[Список идентификаторов продуктов, поддерживаемых средством развертывания Office для запуска](https://learn.microsoft.com/ru-ru/microsoft-365/troubleshoot/installation/product-ids-supported-office-deployment-click-to-run), из списка интересует `ProPlus2024Volume`.

LTSC () --- это версия Офиса для корпоративного сегмента, где после установки в процессе эксплуатации не должен как-либо меняться функционал или даже настройки, поэтому эта версия Офиса с обновлениями безопасности, но без обновления функционала. Поэтому, при возможности выбора, лучше установить полноценную версию; но, если LTSC-версия уже установлена, то и ладно.
